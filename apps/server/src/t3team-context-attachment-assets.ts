...
import { readT3TeamContextAttachments } from "./t3team-context-attachment-parse.ts";
import { hashT3TeamContextBytes } from "./t3team-context-blob-store-utils.ts";
...
export type T3TeamContextAttachmentIndexInfo = {
...
export function buildT3TeamContextAttachmentAssets(input: {
  ...
  return Effect.gen(function* () {
    ...
    const byTicketKey = new Map<string, T3TeamContextAttachmentIndexInfo>();
    ...
    for (const [ticketKey, snapshot] of input.snapshotsByKey) {
      ...
      for (const attachment of readT3TeamContextAttachments(snapshot)) {
