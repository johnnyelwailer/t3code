...
} from "./t3team-toolBrokerPromiseApi.ts";
...
function buildBundledCompatibilityResult(
  ...
  const bundledRecipe = getBundledT3TeamRecipe(manifest.id);
  if (!bundledRecipe) {
    return null;
  }
...
const evaluateVisibleModule = Effect.fn("evaluateVisibleModule")(function* (input: {
  ...
  const toolBroker = Option.getOrElse(
    yield* Effect.serviceOption(T3TeamToolBroker),
    () => NoopT3TeamToolBroker,
  );
  ...
  const tools = binding
    ? createT3TeamPromiseToolApi({ binding, runPromise })
    : createUnavailableT3TeamPromiseToolApi("during visibility evaluation");
