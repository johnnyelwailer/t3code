...
export type T3TeamFixtureTicket = {
  readonly id: string;
...
export type T3TeamFixtureProjectSource = {
  ...
  readonly tickets: ReadonlyArray<T3TeamFixtureTicket>;
  /** Upper-cased key -> ticket, for relationship resolution. */
  readonly ticketsByKey: ReadonlyMap<string, T3TeamFixtureTicket>;
...
function readJsonFile(absolutePath: string): Record<string, unknown> {
  try {
    ...
    throw new T3TeamFixtureProjectSourceError(
...
export function fixtureTicketKey(ticket: T3TeamFixtureTicket): string {
  const displayId = ticket.ref.displayId;
  return String(typeof displayId === "string" && displayId ? displayId : ticket.id).toUpperCase();
}

function readFixtureTickets(fixtureRoot: string): ReadonlyArray<T3TeamFixtureTicket> {
  ...
  const tickets: T3TeamFixtureTicket[] = [];
  for (const entry of NodeFS.readdirSync(workItemsDir).toSorted()) {
    ...
    const ticket = bundle.ticket as T3TeamFixtureTicket | undefined;
    if (ticket?.ref?.id) {
      tickets.push(ticket);
    }
  }
  if (tickets.length === 0) {
    throw new T3TeamFixtureProjectSourceError(`Fixture contains no work items: ${fixtureRoot}`);
  }
  return tickets;
}
 ...
 * on purpose (see `t3team-fixtureProjectRegistry.ts`); only the account id marks it as
...
export function loadT3TeamFixtureProjectSource(input: {
  readonly fixtureRoot: string;
  readonly accountId: string;
}): T3TeamFixtureProjectSource {
  ...
  const ticketsByKey = new Map<string, T3TeamFixtureTicket>();
