...
import {
  ...
  T3workThreadToolContextEvictionReactorLive,
} from "./t3work-threadToolContextEvictionReactor.ts";

const makeTestLayer = (
  fakeEngine: OrchestrationEngineShape,
  putCalls: Array<{ threadId: ThreadId; toolContext?: unknown }>,
) =>
  T3workThreadToolContextEvictionReactorLive.pipe(
    Layer.provideMerge(Layer.succeed(OrchestrationEngineService, fakeEngine)),
    Layer.provideMerge(
      Layer.succeed(T3workThreadToolContextStore, {
...
const runReactor = () =>
  Effect.gen(function* () {
    const reactor = yield* T3workThreadToolContextEvictionReactor;
    yield* Effect.scoped(
      Effect.gen(function* () {
        yield* reactor.start();
...
describe("T3workThreadToolContextEvictionReactorLive", () => {
