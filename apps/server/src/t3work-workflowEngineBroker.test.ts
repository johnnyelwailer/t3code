...
describe("createWorkflowEngineBroker", () => {
  ...
  it("emits BOTH placement halves for a retained child so the sidebar nests it immediately", async () => {
    ...
    const created = placements.find((c) => c.activity.kind === "t3work.handoff.created");
    ...
    // ...and handoff.started on the PARENT (what indexT3workChildParentThreads reads, so the
    // child nests before its own thread detail is ever opened).
    const started = placements.find((c) => c.activity.kind === "t3work.handoff.started");
