...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const first = yield* resolveT3TeamAtlassianViewerAccountId(account);
      ...
      const cached = yield* resolveT3TeamAtlassianViewerAccountId(account);
      ...
      const afterReconnect = yield* resolveT3TeamAtlassianViewerAccountId(account);
      assert.strictEqual(afterReconnect, "new-user");
      assert.strictEqual(requestedUrls.length, 2, "reconnect forces a fresh resolution");
    }).pipe(Effect.provide(testLayer("t3team-atlassian-viewer-identity-invalidate-"))),
);
...
it.effect(
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      const result = yield* resolveT3TeamAtlassianViewerAccountId(account);
      ...
      Effect.provide(
        Layer.mergeAll(
          testLayer("t3team-atlassian-viewer-identity-warn-"),
