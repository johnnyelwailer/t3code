/**
 * The two effort dials on `t3work.thread.start_child`, parsed in one place (split out of
 * `t3work-toolBrokerStartChildArgs.ts` for the additive LOC budget):
 *
...
const REASONING_EFFORTS = new Set<T3workStartChildReasoningEffort>(["low", "medium", "high"]);
...
export const readStartChildReasoningEffort = (
  raw: unknown,
): ParseResult<T3workStartChildReasoningEffort> =>
  parseEnum(
    raw,
    REASONING_EFFORTS,
...
export const readStartChildEffort = (raw: unknown): ParseResult<AgentEffort> =>
  parseEnum(
    raw,
    AGENT_EFFORTS,
    "t3work.thread.start_child 'effort' must be one of 'light', 'standard', or 'high'. " +
