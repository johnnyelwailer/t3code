/**
 * Live wiring for the agent-facing `t3team.workflow.run` tool (ephemeral workflows, slice 1):
...
export function makeWorkflowRunToolHandlers<E>(deps: {
  ...
  return (threadId) => ({
    runWorkflow: (args) => {
      if (!fileSystem || !path) {
        return Effect.fail(
          "Filesystem services are not available for t3team.workflow.run in this runtime.",
        );
      }
...
/** Persist inline `source` under `.t3team-runs/<runId>/workflow.ts`, or containment-check an
 * existing `workflowPath` against the workspace root (same rule as `t3team.recipe.validate`). */
