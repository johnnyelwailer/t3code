...
import type { T3TeamContextAvailability } from "@t3tools/project-context/t3teamContextAvailability";
import * as Effect from "effect/Effect";
import * as FileSystem from "effect/FileSystem";

import { WorkspacePaths } from "./workspace/WorkspacePaths.ts";
import { findIndexedWorkItem } from "./t3team-context-refresh-alias.ts";
import {
  isContextRefreshStale,
  readEntryPointAvailability,
  readManifestSourceUpdatedAt,
  resolveManifestRelativePath,
} from "./t3team-context-refresh-freshness.ts";
import {
  readWorkItemSummariesByRelativePath,
  readWorkItemsIndex,
} from "./t3team-context-refresh-index.ts";
import { loadT3TeamContextProjectMetadata } from "./t3team-context-refresh-metadata.ts";
import { normalizeTicketKey } from "./t3team-toolBrokerContextSyncScope.ts";

export type T3TeamContextRefreshScope = {
...
export function loadT3TeamContextRefreshScope(input: {
  ...
  return Effect.gen(function* () {
    ...
    const project = yield* loadT3TeamContextProjectMetadata({
      workspaceRoot: input.workspaceRoot,
    ...
    return {
      ...
      availability,
      stale,
    } satisfies T3TeamContextRefreshScope;
  });
}
