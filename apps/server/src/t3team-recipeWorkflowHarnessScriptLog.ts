 ...
 * (`t3team-sdk.toolScriptCalls.ts`) and journals a `script` / `script-never` primitive entry
 ...
 *
 * A declared-but-never-dispatched script now shows up in {@link T3TeamHarnessScriptLog.uncalledScripts},
...
export type T3TeamHarnessScriptLog = {
...
export function buildT3TeamHarnessScriptLog(input: {
...
export async function readT3TeamHarnessScriptLog(input: {
  readonly store: JournalStore;
  readonly runId: string;
  readonly declaredScripts: ReadonlyArray<string>;
}): Promise<T3TeamHarnessScriptLog> {
  const maps = await input.store.readEntries(input.runId);
  return buildT3TeamHarnessScriptLog({
