...
const makeTempWorkspace = Effect.fn("makeTempWorkspace")(function* () {
  ...
  return yield* fileSystem.makeTempDirectoryScoped({
    ...
    prefix: "t3work-recipe-agent-validate-",
  });
});
...
describe("validateProjectRecipeWorkflowForAgent", () => {
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/valid/valid.workflow.ts",
            content: VALID_WORKFLOW,
          });

          const result = yield* validateProjectRecipeWorkflowForAgent({
            workspaceRoot,
            path: ".t3work/recipes/valid/valid.workflow.ts",
          });
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/broken/broken.workflow.ts",
            content: BROKEN_META_WORKFLOW,
          });

          const result = yield* validateProjectRecipeWorkflowForAgent({
            workspaceRoot,
            path: ".t3work/recipes/broken/broken.workflow.ts",
          });
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/typed-dir/typed-dir.workflow.ts",
            content: VALID_WORKFLOW,
          });
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/typed-dir/recipe.ts",
            content: [
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/legacy-dir/legacy-dir.workflow.ts",
            content: VALID_WORKFLOW,
          });
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/legacy-dir/prompt.md",
            content: "Do the thing.",
          });
          yield* writeWorkspaceFile({
            workspaceRoot,
            relativePath: ".t3work/recipes/legacy-dir/recipe.json",
            content: `{
  "id": "legacy-dir",
  "version": "0.1.0",
  "scope": "project",
  ...
  "workflow": "./legacy-dir.workflow.ts"
          ...
          const result = yield* validateProjectRecipeWorkflowForAgent({
            workspaceRoot,
            path: ".t3work/recipes/legacy-dir",
          });
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const fileSystem = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const emptyDir = path.join(workspaceRoot, ".t3work/recipes/empty-dir");
          yield* fileSystem.makeDirectory(emptyDir, { recursive: true });

          const result = yield* validateProjectRecipeWorkflowForAgent({
            workspaceRoot,
            path: ".t3work/recipes/empty-dir",
          });
