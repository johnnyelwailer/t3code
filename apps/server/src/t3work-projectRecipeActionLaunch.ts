 ...
 * authorization uses ({@link ./t3work-workflowRunPackAuthorize.ts}). No name (or `"default"`) keeps
...
export class T3workRecipeActionResolutionError extends Data.TaggedError(
  "T3workRecipeActionResolutionError",
)<{
...
export const resolveLaunchWorkflowPath = Effect.fn("resolveLaunchWorkflowPath")(function* (input: {
  ...
  if (recipePath.length === 0) {
    return yield* new T3workRecipeActionResolutionError({
  ...
  const ref = yield* importRecipeModuleRef(modulePath).pipe(
    Effect.mapError(
      (error) =>
        new T3workRecipeActionResolutionError({
  ...
  return yield* Effect.try({
    ...
    catch: (error) => new T3workRecipeActionResolutionError({ message: errorMessage(error) }),
  });
});
