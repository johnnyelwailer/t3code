...
/**
 * Execution authorization for `t3work.orchestration.run`. A pack's recipe root is NOT an
...
describe("resolveRunWorkflowPath — execution authorization", () => {
  it("runs a pack recipe's DECLARED workflow but refuses another .ts under the same recipe root", async () => {
    await run(
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const packDir = yield* makeTempDir("t3work-run-auth-pack-");
        const workspaceRoot = yield* makeTempDir("t3work-run-auth-ws-");
  ...
  it("refuses a SHADOWED pack recipe's workflow while the project-local one stays runnable", async () => {
    await run(
      Effect.gen(function* () {
        ...
        const localRecipeRoot = path.join(workspaceRoot, ".t3work/recipes/triage");
  ...
  it("refuses a symlinked pack recipe root and a symlink nested under a valid root", async () => {
    await run(
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const packDir = yield* makeTempDir("t3work-run-auth-pack-");
        const outsideDir = yield* makeTempDir("t3work-run-auth-outside-");
        const workspaceRoot = yield* makeTempDir("t3work-run-auth-ws-");
        ...
        const packDir2 = yield* makeTempDir("t3work-run-auth-pack2-");
  ...
  it("keeps writing and accepting the ephemeral .t3work-runs/<runId>/workflow.ts source", async () => {
    await run(
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const workspaceRoot = yield* makeTempDir("t3work-run-auth-ws-");
        ...
        const expected = path.join(workspaceRoot, ".t3work-runs", "run-eph", "workflow.ts");
