...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  ...
  // Fork policy is origin-first (see pickPrimaryRemote), so the refresh scenario adds or
  // replaces `origin` rather than upstream's `upstream`.
  it.effect.each(["add", "replace"] as const)(
    "refreshes the primary remote after %s before cache expiry",
    (change) =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "t3-repository-identity-refresh-test-",
        });

        yield* git(cwd, ["init"]);
        yield* git(cwd, ["remote", "add", "upstream", "git@github.com:T3Tools/t3code.git"]);
        if (change === "replace") {
          yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/previous.git"]);
        }

        const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
        const initialIdentity = yield* resolver.resolve(cwd);
        expect(initialIdentity?.canonicalKey).toBe(
          change === "add" ? "github.com/t3tools/t3code" : "github.com/julius/previous",
        );
