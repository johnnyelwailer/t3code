/**
 * The exact provider/model-selection help topic (`help("model-selection")`), split out of
 ...
 * `models` tool instead of any static catalog.
 */
...
exact target, call models and use one returned instanceId + model slug verbatim. Never copy
ids from examples or guess from a provider family name. Then construct the typed value generically:
  ...
  const selected = {
    provider: '<instanceId returned by models>',
    model: defineModel({
...
starts. Omit model to inherit the currentSelection reported by models. Prefer effort:
