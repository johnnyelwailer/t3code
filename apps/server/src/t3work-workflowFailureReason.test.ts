/* oxlint-disable t3code/no-manual-effect-runtime-in-tests -- mirrors t3work-toolBrokerWorkflowResumeTool.test.ts: a real-engine integration test bridging the Effect runtime. */
 ...
 *
 * `t3work_orchestration_status` / `_resume` used to return a bare status, so an agent (or the
...
import { makeWorkflowStatusToolHandlers } from "./t3work-toolBrokerWorkflowStatusTool.ts";
import {
...
const cwd = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3work-failure-reason-"));
...
const TestLayer = Layer.mergeAll(
  T3workWorkflowSchedulerLive.pipe(
    Layer.provideMerge(
      Layer.mergeAll(
        T3workWorkflowEngineRegistryLive,
