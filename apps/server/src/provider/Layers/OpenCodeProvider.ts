  ...
  const inventoryExit = yield* Effect.exit(
    (isExternalServer
      ...
      : openCodeRuntime.loadInventoryFromCli({
          ...
          // set, otherwise an empty override would hide the user's own OpenCode config.
          environment: openCodeSettings.configContent?.trim()
            ? {
                ...resolvedEnvironment,
                OPENCODE_CONFIG_CONTENT: openCodeSettings.configContent,
              }
            : resolvedEnvironment,
        })
    ).pipe(
