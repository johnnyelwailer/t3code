...
describe("EventNdjsonLogger", () => {
  ...
  // Regression: the store keeps one sink per thread segment, and before the idle
  // sweep that map only ever grew — a long-lived server accumulated an entry for
  // every thread it had ever logged. Eviction must be transparent: a later write
  // for an evicted thread re-creates its sink and keeps appending to the same file.
  it.effect("evicts sinks idle past the TTL and re-creates them on the next write", () =>
    Effect.gen(function* () {
      const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3-provider-log-"));
      const basePath = NodePath.join(tempDir, "provider-native.ndjson");
      ...
      try {
        ...
        // Any later drain sweeps segments idle past the TTL.
