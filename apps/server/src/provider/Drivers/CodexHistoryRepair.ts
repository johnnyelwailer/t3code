...
const buildHistoryPlan = Effect.fn("buildCodexHistoryPlan")(function* (
  ...
  for (const filePath of directories.flat()) {
    ...
    const scan = scanCodexRolloutFile(filePath, originalContents, input.providerThreadId);
...
export const inspectCodexThreadHistory = Effect.fn("inspectCodexThreadHistory")(function* (
  ...
  FileSystem.FileSystem | Path.Path
> {
  return (yield* buildHistoryPlan(input)).report;
});
...
export const repairCodexThreadHistory = Effect.fn("repairCodexThreadHistory")(function* (
  ...
  FileSystem.FileSystem | Path.Path
> {
  const plan = yield* buildHistoryPlan(input);
  if (
    plan.report.status === "unsafe" ||
    plan.report.status === "not-found" ||
    plan.report.status === "clean"
  ) {
    return plan.report;
  }
