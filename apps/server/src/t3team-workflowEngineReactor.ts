...
export const T3TeamWorkflowEngineReactorLive = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const processMessageSent = Effect.fn("processWorkflowEngineMessageSent")(function* (
      ...
      if (role === "user" && event.payload.t3teamExt?.widgetReply !== undefined) return;
      ...
      const workflowReply = role === "user" ? event.payload.t3teamExt?.workflowReply : undefined;
      if (
      ...
      // one (`t3teamExt.workflowReply`, e.g. a decision-card choice), else the message text.
    ...
    const processSafely = (event: ThreadMessageSentEvent) =>
      processMessageSent(event).pipe(
        Effect.catchCause((cause) => {
          ...
          return Effect.logWarning("t3team workflow-engine reactor failed to process event", {
