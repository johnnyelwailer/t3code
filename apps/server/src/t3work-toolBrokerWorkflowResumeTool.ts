...
import { makeResumeFailedRun } from "./t3work-toolBrokerWorkflowResumeFailed.ts";
...
export type T3workWorkflowResumeToolHandlers = {
  ...
  return (threadId) => ({
    resumeWorkflowRun: (args) =>
      Effect.gen(function* () {
        ...
        return yield* makeResumeFailedRun(deps, threadId, () => t3workRandomUUID())(run);
      }),
  });
}
