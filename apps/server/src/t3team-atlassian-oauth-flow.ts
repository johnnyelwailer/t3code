...
export type { AtlassianOAuthFlowStatus };

/**
 * Answers "did flow `state` finish?" for a tab that has no other way to find out — no opener to
 * `postMessage` it, no same-origin broadcast, because sign-in completed in a different browser
 * entirely. Unauthenticated for the same reason `completeAtlassianOAuthFlow` is: `state` is the whole
 * capability already, generated by this server's CSPRNG, and the response here carries even less than
 * that route's — no tokens, no accounts, just which of three states one opaque string is in.
 */
export function resolveAtlassianOAuthFlowStatus(state: string) {
  return Effect.gen(function* () {
    const nowMs = yield* Clock.currentTimeMillis;
    return readAtlassianOAuthFlowStatus(state, nowMs);
  });
}
