...
import { clearT3TeamFixtureProjects } from "./t3team-fixtureProjectRegistry.ts";
import { seedT3TeamFixtureProject } from "./t3team-fixtureProjectSeed.ts";
import { makeContextRefreshIntegrationTestLayer } from "./t3team-contextRefreshTestFixtures.ts";

const tempRoots: string[] = [];

afterEach(() => {
  clearT3TeamFixtureProjects();
...
function makeFixtureDirectory(): string {
  const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3team-fixture-source-"));
  tempRoots.push(root);
...
it.effect("ingests a fixture directory through the live refresh pipeline", () =>
  Effect.gen(function* () {
    ...
    const workspaceRoot = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "t3team-fixture-workspace-"),
    );
    tempRoots.push(workspaceRoot);

    const seeded = yield* seedT3TeamFixtureProject({ fixtureRoot, workspaceRoot });
    ...
    }>`SELECT resource_key, snapshot_json FROM t3team_context_resources ORDER BY resource_key`;
    assert.deepEqual(
    ...
    const edges = yield* sql<{
      ...
      target_key: string;
      relation: string;
    }>`SELECT source_key, target_key, relation FROM t3team_context_edges ORDER BY source_key, relation`;
    ...
    const search = yield* sql<{
      count: number;
    }>`SELECT COUNT(*) AS count FROM t3team_context_search`;
    ...
    Effect.provide(
      Layer.mergeAll(
        makeContextRefreshIntegrationTestLayer("t3team-fixture-project-seed-"),
        NodeServices.layer,
        WorkspacePaths.layer.pipe(Layer.provide(NodeServices.layer)),
        ServerConfig.layerTest(process.cwd(), { prefix: "t3team-fixture-project-seed-" }).pipe(
