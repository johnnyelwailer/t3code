/**
 * Pack-shipped recipes for the agent-facing `t3work.recipe.list`.
 *
 ...
 * recipe roots the UI path uses ({@link ./t3work-packRecipeSources.ts}) — not a second scan of the
 * project tree. Loading goes through the shared per-directory loader
 * ({@link ./t3work-recipeAgentListEntry.ts}), so a pack recipe yields the same entry shape as a
