 ...
 * t3work-atlassian-backlog-mirrorSyncService.ts). Split into its own module
...
export function recordT3workMirrorSyncKick(mapKey: string): void {
...
export function lastT3workMirrorSyncKickMs(mapKey: string): number | undefined {
  return lastKickedMsByKey.get(mapKey);
}

/** Forget all kick history (e.g. when every active loop is being stopped). */
export function clearT3workMirrorSyncKickHistory(): void {
 ...
 */
export function isT3workMirrorSyncIdle(input: {
