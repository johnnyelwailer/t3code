...
/**
 * The agent-facing recipe tools must see PACK-SHIPPED recipes, not just `.t3work/recipes/*`.
 *
 * The library the system prompt tells the model to prefer ("run a fitting saved recipe by `path`
 * rather than re-authoring it") ships as pack content, so a `t3work_recipe_list` that enumerated
 * only project-local recipes made that advice unactionable — and `t3work_recipe_validate` rejected
...
describe("listProjectRecipesForAgent with pack-shipped recipes", () => {
  it("returns a pack recipe with its source label and a usable path, with no project-local recipes at all", async () => {
    await run(
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const packDir = yield* makeTempDir("t3work-agent-list-pack-");
        const workspaceRoot = yield* makeTempDir("t3work-agent-list-ws-");
  ...
  it("keeps project-local recipes and lets one shadow a same-id pack recipe, with a diagnostic", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(triage?.recipePath).toBe(path.join(workspaceRoot, ".t3work/recipes/triage"));
