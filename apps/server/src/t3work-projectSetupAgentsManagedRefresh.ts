...
export function renderPreviousAgentsMdOfferFirst(profile: ProjectSetupProfileDefinition): string {
  ...
  // Reproduces the AGENTS.md version replaced by the t3work-workflow-engine-ax-ux pass that
...
- A t3work recipe is a project-scoped, reusable action saved under ${T3WORK_PROJECT_RECIPES_ROOT}/<id>/ -- a \`recipe.ts\` (\`defineRecipe\`) plus a typed \`<id>.workflow.ts\` (\`defineWorkflow\`) that t3work itself discovers, renders a launch surface for, and runs durably -- it can pause for a user decision, wait on the clock, and survive restarts.
- It is not a Claude Code skill, slash command, or subagent, and not a Codex or CI workflow. Those live in the provider or tooling layer; a t3work recipe lives in the product and shows up as a launchable action in the t3work UI.
- If the user says "workflow" in a t3work project, default to reading it as a t3work recipe/workflow unless they clearly mean a provider or CI feature; ask when it's ambiguous.
