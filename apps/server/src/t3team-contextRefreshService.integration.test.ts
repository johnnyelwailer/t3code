...
it.effect("refreshes a work-item bundle server-side without browser sync", () =>
  Effect.gen(function* () {
    ...
    const service = yield* T3TeamContextRefreshService;
    const input = {
    ...
    assert.equal(refreshed.availability, T3TEAM_CONTEXT_AVAILABILITY_FULL);
    ...
    Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-service-")),
  ),
);
...
it.effect("builds a multi-node foreground graph for linked mock issues", () =>
  ...
  }).pipe(Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-graph-"))),
);
...
it.effect("persists background refresh jobs after a foreground sync", () =>
  Effect.gen(function* () {
    ...
    const job = yield* loadT3TeamContextRefreshJob(refreshed.backgroundJobId!);
    assert.isDefined(job);
    assert.equal(job!.rootKey, refreshed.ticketKey.toUpperCase());
  }).pipe(Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-bg-"))),
);
...
it.effect("completes background refresh jobs after a foreground sync", () =>
  Effect.gen(function* () {
    ...
    const finalJob = yield* loadT3TeamContextRefreshJob(refreshed.backgroundJobId!);
    assert.strictEqual(finalJob?.status, "completed");
  }).pipe(
    Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-bg-complete-")),
  ),
);
...
it.effect("force refresh supersedes an in-flight refresh for the same ticket", () =>
  ...
  ).pipe(Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-force-"))),
);
...
it.effect("force refresh rebuilds an already_synced bundle", () =>
  Effect.gen(function* () {
    ...
    Effect.provide(makeContextRefreshIntegrationTestLayer("t3team-context-refresh-rebuild-")),
  ),
);
