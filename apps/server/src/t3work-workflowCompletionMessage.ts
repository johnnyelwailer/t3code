...
async function postTerminalMessage(input: {
  ...
  await input
    .dispatch({
      ...
      commandId: CommandId.make(`t3work-wf:${input.kind}:${input.newId()}`),
    ...
    .catch((error: unknown) => {
      ...
      console.warn(
        `[t3work-workflow] failed to deliver terminal ${input.kind} message for run ${input.workflowRunId}:`,
        error,
      );
    });
}
...
export async function deliverWorkflowFailure(input: {
  ...
  await postTerminalMessage({
    ...
    workflowRunId: input.workflowRunId,
    kind: "failed",
    text: `⚠️ Workflow run failed${reason.length > 0 ? `: ${reason}` : "."}\n\nThe run is no longer active. Fix the orchestration source and launch it again — call t3work_help("agent-orchestration") for the authoring format.`,
    dispatch: input.dispatch,
    newId: input.newId,
    nowIso: input.nowIso,
  });
