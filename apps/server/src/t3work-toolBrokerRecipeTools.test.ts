...
describe("makeRecipeToolHandlers", () => {
  it("fails with a clear error when the thread's project has no workspace root", async () => {
    ...
    if (result._tag === "Failure") {
      expect(result.failure).toBe("Current t3work project has no workspace root.");
    }
  });
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const workspaceRoot = yield* fileSystem.makeTempDirectoryScoped({
            prefix: "t3work-recipe-tool-handlers-",
          });
          const recipeRoot = path.join(workspaceRoot, ".t3work/recipes/legacy-recipe");
