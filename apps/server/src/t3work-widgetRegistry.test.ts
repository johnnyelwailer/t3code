...
import { T3workWidgetToolCallRequest } from "@t3tools/contracts";

import { createT3workWidgetRegistry } from "./t3work-widgetRegistry.ts";

const run = <A>(effect: Effect.Effect<A>) => Effect.runPromise(effect);
const decode = Schema.decodeUnknownEffect(T3workWidgetToolCallRequest);

describe("createT3workWidgetRegistry", () => {
...
describe("T3workWidgetToolCallRequest", () => {
  it("accepts a well-formed body", async () => {
    const decoded = await run(
      decode({ threadId: "t1", widgetId: "w1", tool: "t3work.view.read" }).pipe(Effect.result),
    );
