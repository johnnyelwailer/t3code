 ...
 *
 * The background loop (kickT3TeamAtlassianMirrorSync) uses forkDetach which
...
import {
  ...
  nextMirrorSleepMs,
  hasActiveT3TeamAtlassianMirrorSync,
...
const mirrorSyncTestLayer = Layer.mergeAll(
  ...
  NodeServices.layer,
  ServerConfig.layerTest(process.cwd(), { prefix: "t3team-mirror-sync-test" }).pipe(
...
mirrorSyncCacheLayer("t3team Atlassian mirror sync service", (it) => {
  it.effect(
...
describe("isT3TeamMirrorSyncIdle", () => {
  it("is not idle while within the TTL", () => {
    const nowMs = 10 * 60_000;
    assert.strictEqual(isT3TeamMirrorSyncIdle({ nowMs, lastKickedMs: nowMs - 29 * 60_000 }), false);
  });

  it("is idle once the gap since the last kick reaches the 30 m TTL", () => {
    const nowMs = 60 * 60_000;
    assert.strictEqual(isT3TeamMirrorSyncIdle({ nowMs, lastKickedMs: nowMs - 30 * 60_000 }), true);
  });
});
...
// existing single-flight test's comment explains). `isT3TeamMirrorSyncIdle`
// above covers the actual decision logic in isolation instead.

it.live("stopAllT3TeamAtlassianMirrorSyncs releases every registered loop key", () =>
  Effect.gen(function* () {
    const request = {
      account: {
    ...
    yield* kickT3TeamAtlassianMirrorSync(request);
    assert.ok(hasActiveT3TeamAtlassianMirrorSync(request));
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* kickT3TeamAtlassianMirrorSync(request);
      yield* waitUntil(() => !hasActiveT3TeamAtlassianMirrorSync(request));
      assert.ok(
        !hasActiveT3TeamAtlassianMirrorSync(request),
