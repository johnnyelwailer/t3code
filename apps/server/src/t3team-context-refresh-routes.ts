...
export const t3teamProjectWorkspaceRefreshProjectContextRouteLayer = HttpRouter.add(
  "POST",
  "/api/t3team/project/workspace/context-refresh/project",
  Effect.gen(function* () {
    const refreshService = yield* T3TeamContextRefreshService;
    ...
    const result = yield* refreshService.refreshProject({
      threadId: ThreadId.make("t3team-context-refresh-http"),
      projectId,
      workspaceRoot,
    ...
    Effect.mapError((cause) => toT3TeamError(cause, "Failed to refresh project context.")),
    Effect.catch(errorResponse),
  ),
);

export const t3teamProjectWorkspaceRefreshWorkItemContextRouteLayer = HttpRouter.add(
  "POST",
  ...
  Effect.gen(function* () {
    ...
    Effect.mapError((cause) => toT3TeamError(cause, "Failed to refresh work-item context.")),
    Effect.catch(errorResponse),
  ),
);

export const t3teamProjectWorkspaceRefreshWorkItemSliceContextRouteLayer = HttpRouter.add(
  "POST",
  ...
  Effect.gen(function* () {
    ...
    Effect.mapError((cause) => toT3TeamError(cause, "Failed to refresh work-item slice context.")),
    Effect.catch(errorResponse),
  ),
);
