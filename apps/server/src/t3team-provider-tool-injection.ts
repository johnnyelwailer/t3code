...
export interface T3TeamRawToolInjectionConfig {
...
export interface T3TeamProviderToolInjectionPlan {
  readonly claudeMcpServers?: ReadonlyArray<unknown>;
  readonly cursorMcpServers?: ReadonlyArray<unknown>;
...
export const T3TEAM_RAW_TOOL_INJECTION_ENV = "T3TEAM_RAW_TOOL_INJECTION_JSON";
...
export function resolveT3TeamRawToolInjectionConfig(
  environment?: NodeJS.ProcessEnv,
): T3TeamRawToolInjectionConfig | undefined {
  const raw = environment?.[T3TEAM_RAW_TOOL_INJECTION_ENV]?.trim();
...
export function buildT3TeamProviderToolInjectionPlan(
  environment?: NodeJS.ProcessEnv,
): T3TeamProviderToolInjectionPlan {
  const resolved = resolveT3TeamRawToolInjectionConfig(environment);
