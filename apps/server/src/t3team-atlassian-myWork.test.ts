 ...
 * t3team-atlassian-auth-store.test.ts) so `providerForAccount` resolves a
...
const myWorkLayer = it.layer(
  Layer.mergeAll(
    ...
    NodeServices.layer,
    ServerConfig.layerTest(process.cwd(), { prefix: "t3team-atlassian-my-work-test" }).pipe(
...
myWorkLayer("t3team Atlassian My Work page selection", (it) => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const page = yield* loadT3TeamAtlassianMyWorkPage({
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const first = yield* loadT3TeamAtlassianMyWork({
          account,
          externalProjectId: "project-fp",
          poll: { enabled: true },
        });
        ...
        const second = yield* loadT3TeamAtlassianMyWork({
          account,
          externalProjectId: "project-fp",
