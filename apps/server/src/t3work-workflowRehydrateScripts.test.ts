...
// The recipe workspace lives under __fixtures__ so the recipe module's `@t3work/sdk` import
// resolves through the package's node_modules (same trick as t3work-recipeWorkflowScripts.test).
const workspaceRoot = NodeFS.mkdtempSync(
  NodePath.join(fixtureRoot, "t3work-rehydrate-scripts-workspace-"),
);
const cwd = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3work-rehydrate-scripts-"));
afterAll(() => {
...
function writeRecipeFixture(recipeId: string): { recipeRoot: string; workflowPath: string } {
  const recipeRoot = NodePath.join(workspaceRoot, ".t3work", "recipes", recipeId);
  ...
  Effect.gen(function* () {
    ...
    const launched = yield* Effect.promise(() =>
      launchWorkflowRecipe({
        ...
        args: { ratios: [1, 2, 3] },
        scripts,
        runsRoot: NodePath.join(config.cwd, ".t3work-runs"),
