...
import type { AuthState as ToolAuthInternalState } from "./toolauth/t3team-types.ts";
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const toShellStreamEvent = (
        ...
        // Ephemeral threads are filtered here and everything else delegates to
        // `threadUpsertOrRemove`, so the upstream retry and upsert-or-remove
        // semantics stay in one place. A failed retention read is treated as
        // retained: dropping the event would strand the sidebar instead.
        const retainedThreadShellUpsert = (threadId: ThreadId) =>
          projectionSnapshotQuery.getThreadDetailById(threadId).pipe(
            Effect.map((detail) => Option.isSome(detail) && detail.value.retention === "ephemeral"),
            Effect.orElseSucceed(() => false),
            Effect.flatMap((isEphemeral) =>
              isEphemeral
                ? Effect.succeed(Option.none<OrchestrationShellStreamEvent>())
                : threadUpsertOrRemove(threadId, event.sequence),
            ),
          );
        switch (event.type) {
          case "project.created":
