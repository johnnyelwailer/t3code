...
describe("T3TeamToolBroker allowed tool groups", () => {
  ...
  it("allows backlog view-state tools when the recipe declares view.state", async () => {
    ...
    expect(result.binding?.listServers()[0]?.tools).toEqual({
      "t3team.backlog.set_assignee_filter": expect.objectContaining({
        name: "t3team.backlog.set_assignee_filter",
      }),
    });
      ...
      Effect.gen(function* () {
        ...
        const binding = yield* broker.bindSession({
          ...
          toolContext: {
            ...
            tools: [
              {
                id: "t3team.work_item.assignee.draft_update",
      ...
      Effect.gen(function* () {
        ...
        const binding = yield* broker.bindSession({
          ...
          toolContext: {
            ...
            tools: [
              {
                id: "t3team.work_item.estimate.draft_update",
