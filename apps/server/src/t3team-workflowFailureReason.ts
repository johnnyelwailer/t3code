 ...
 * fallback phrase already used in the status tool's hint so the two surfaces agree on wording. */
 ...
 * to a user (GHE #344: `resume: thread.turn (QA round 1)` → `thread.turn (QA round 1)`). The raw
 * value — phase prefix included — stays in the DB row and in agent-facing surfaces
 * (`t3team.orchestration.status` / `.resume`), which need the phase to reason about where a run
 * parked. A string with no phase prefix passes through unchanged; a bare phase token strips to
 * the generic label instead of leaking the internal phase name. */
