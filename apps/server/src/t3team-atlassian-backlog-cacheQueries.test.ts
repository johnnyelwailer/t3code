...
cacheQueriesLayer("t3team Atlassian My Work projection query", (it) => {
  it.effect("returns only issues assigned to the viewer plus one level of missing parents", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(projection.assigned.map((item) => item.displayId).sort(), [
        "PROJ-1",
        "PROJ-2",
      ]);
  ...
  it.effect("does not re-fetch a parent that is already in the assigned set", () =>
    Effect.gen(function* () {
      yield* writeCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          first.assigned.map((item) => item.displayId),
          expectedOrder,
        );
        ...
        assert.deepStrictEqual(
          second.assigned.map((item) => item.displayId),
          expectedOrder,
        );
      }),
  );
...
hasMirrorRowsLayer("t3team Atlassian mirror hasMirrorRowsForProject", (it) => {
  ...
  it.effect("returns true once rows are mirrored, even if none are assigned to the viewer", () =>
    Effect.gen(function* () {
      yield* writeCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
