...
import { parseWorkItemsIndex } from "./t3team-toolBrokerContextSyncScope.ts";

export type T3TeamContextProjectRefreshScope = {
...
export function loadT3TeamContextProjectRefreshScope(input: {
  ...
  return Effect.gen(function* () {
    ...
    const indexPath = yield* workspacePaths.resolveRelativePathWithinRoot({
      workspaceRoot: input.workspaceRoot,
      relativePath: T3TEAM_WORK_ITEMS_INDEX_PATH,
    });
    ...
    return {
      ...
      workItemCount,
      stale,
    } satisfies T3TeamContextProjectRefreshScope;
  });
}
