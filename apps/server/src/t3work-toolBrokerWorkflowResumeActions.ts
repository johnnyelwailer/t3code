 ...
 * ./t3work-toolBrokerWorkflowResumeFailed.ts (additive size budget).
 */
...
import { replaceEphemeralWorkflowSourceAtomically } from "./t3work-workflowEphemeralSource.ts";
...
export const workspaceRootFor = <E>(deps: WorkflowResumeToolDeps<E>, threadId: ThreadId) =>
  Effect.gen(function* () {
    ...
    if (typeof project.workspaceRoot !== "string" || project.workspaceRoot.length === 0) {
      return yield* Effect.fail("Current t3work project has no workspace root.");
    }
 ...
 * `.t3work-runs/<runId>/workflow.ts`, re-read on every resume). */
export const replaceRunSourceIfRequested = <E>(
  ...
  Effect.gen(function* () {
    ...
    const runsRoot = deps.path.join(workspaceRoot, ".t3work-runs");
    const ephemeralPath = deps.path.join(runsRoot, run.runId, "workflow.ts");
    if (run.workflowPath !== ephemeralPath) {
      return yield* Effect.fail(
        "Corrected source is only supported for ephemeral runs (source under .t3work-runs); " +
