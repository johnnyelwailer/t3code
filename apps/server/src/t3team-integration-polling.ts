...
export function createT3TeamPollFingerprint(value: unknown): string {
  return `sha256:${NodeCrypto.createHash("sha256")
    .update(JSON.stringify(canonicalizeForFingerprint(value)))
    .digest("hex")}`;
}

export function toT3TeamPollResult<T>(value: T, poll: T3TeamPollEnvelope): T3TeamPollResult<T> {
  const fingerprint = createT3TeamPollFingerprint(value);
