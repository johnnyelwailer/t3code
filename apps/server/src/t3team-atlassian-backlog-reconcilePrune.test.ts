...
import {
  pruneStaleT3TeamAtlassianBacklogViews,
  T3TEAM_BACKLOG_VIEW_STALE_MS,
} from "./t3team-atlassian-backlog-reconcilePrune.ts";
...
viewsLayer("pruneStaleT3TeamAtlassianBacklogViews", (it) => {
  it.effect("deletes only rows older than the stale threshold", () =>
    Effect.gen(function* () {
      ...
      yield* insertView({
        identity,
        selectionKey: "stale",
        updatedAt: nowMs - T3TEAM_BACKLOG_VIEW_STALE_MS - 1,
      });
      yield* insertView({ identity, selectionKey: "fresh", updatedAt: nowMs - 1_000 });

      const pruned = yield* pruneStaleT3TeamAtlassianBacklogViews({ identity, nowMs });
  ...
  it.effect("respects a custom staleMs override", () =>
    Effect.gen(function* () {
      ...
      const pruned = yield* pruneStaleT3TeamAtlassianBacklogViews({
        identity,
        nowMs,
        staleMs: 5_000,
      });
  ...
  it.effect("scopes deletion to the given project only", () =>
    Effect.gen(function* () {
      ...
      const nowMs = 1_000_000_000_000;
      const staleUpdatedAt = nowMs - T3TEAM_BACKLOG_VIEW_STALE_MS - 1;
...
describe("T3TEAM_BACKLOG_VIEW_STALE_MS", () => {
  it("is 30 days", () => {
    assert.strictEqual(T3TEAM_BACKLOG_VIEW_STALE_MS, 30 * 24 * 60 * 60 * 1000);
  });
});
