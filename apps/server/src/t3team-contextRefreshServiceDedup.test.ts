...
import {
  ...
  makeT3TeamContextRefreshSupersession,
...
function refreshInput(force: boolean): T3TeamContextRefreshInput {
  return {
...
function syncedResult() {
  return {
    ...
    entryPointRelativePath: ".t3team/context/entrypoint.json",
    manifestRelativePath: ".t3team/context/manifest.json",
    includedCount: 1,
    skippedCount: 0,
  };
}
...
it.effect("makeT3TeamContextRefreshSupersession abort stops refresh checkpoints", () =>
  Effect.gen(function* () {
    const supersession = makeT3TeamContextRefreshSupersession();
    yield* assertT3TeamContextRefreshNotSuperseded(supersession);
    supersession.abort();
    const exit = yield* assertT3TeamContextRefreshNotSuperseded(supersession).pipe(Effect.exit);
