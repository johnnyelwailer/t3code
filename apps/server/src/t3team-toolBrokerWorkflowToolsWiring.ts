/**
 * Combines the `t3team.workflow.run` and `t3team.workflow.status` per-thread wiring into one
 * call. Both are small `*Live.ts` builders that already resolve their own optional durable-engine
 * deps and return `undefined` when unwired; this just gives `t3team-toolBrokerLive.ts` (already
