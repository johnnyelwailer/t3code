...
it.effect("answers a known pull request immediately while the host refreshes", () =>
  Effect.gen(function* () {
    ...
    // t3team: resolving the ref now reads the project's linked-repository file
    // (real fs I/O) before the host call, so the background refresh needs more
    // than one scheduler tick to reach the provider.
