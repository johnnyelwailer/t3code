...
export const make = Effect.gen(function* () {
  ...
  const listWorkspaceProjects = (
    ...
    (filter.projectId === undefined
      ...
      Effect.flatMap(({ refinedProviders, projects, visible }) =>
      ...
      Effect.map(({ refinedProviders, projects, linkedByProject }) => {
        ...
        const addProjectRepository = (
          ...
          const key = listCursorKey(host, cursorRepository);
        ...
        for (const project of projects) {
          ...
          if (!identity || kind === undefined || repository === null) {
            // No remote of its own, but the project's linked repositories still list.
          ...
          for (const linked of linkedByProject.get(project.id) ?? []) {
            if (filter.host !== undefined && linked.host !== filter.host.toLowerCase()) continue;
            addProjectRepository(project, linked.host, linked.kind, linked.repository, linked.repository);
          }
        }
  ...
  const requireProject = (ref: PullRequestRef): Effect.Effect<SupportedProject, PullRequestError> =>
    listWorkspaceProjects({ projectId: ref.projectId }).pipe(
      Effect.flatMap(({ supported }): Effect.Effect<SupportedProject, PullRequestError> => {
        ...
        // A project's linked repositories (t3team) are checked against the reference exactly
        // as its own remote is, rather than being handed to a provider verbatim.
        const linked = supported.find(
          (candidate) =>
            candidate.repository.toLowerCase() === repository.toLowerCase() &&
            (host === undefined || candidate.host === host),
        );
        if (linked !== undefined) return Effect.succeed(linked);
        if (host === undefined) {
          if (own === undefined) {
