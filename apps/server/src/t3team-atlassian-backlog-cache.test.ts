...
function createBacklogPayload(
  overrides?: Partial<T3TeamAtlassianBacklogPayload>,
  ...
  return {
    ...
    } satisfies T3TeamAtlassianBacklogCapabilities,
...
backlogCacheLayer("t3team Atlassian backlog cache", (it) => {
  it.effect("persists raw issue rows and resolves both request and resolved selections", () =>
    Effect.gen(function* () {
      ...
      const requestCached = yield* readCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
        externalProjectId: "project-1",
      });
      ...
      const resolvedCached = yield* readCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
        externalProjectId: "project-1",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const withQuickFilters = yield* readCachedT3TeamAtlassianBacklog({
          provider: "atlassian",
          accountId: "account-1",
          externalProjectId: "project-1",
        ...
        const withoutQuickFilters = yield* readCachedT3TeamAtlassianBacklog({
          provider: "atlassian",
          accountId: "account-1",
          externalProjectId: "project-1",
  ...
  it.effect("appends sync pages, advances the cursor, and prunes on walk completion", () =>
    Effect.gen(function* () {
      ...
      const midWalk = yield* readCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
        externalProjectId: "project-1",
      });
      ...
      const completed = yield* readCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
        externalProjectId: "project-1",
      });
  ...
  it.effect("updates view metadata columns without touching persisted issue ids or cursor", () =>
    Effect.gen(function* () {
      yield* writeCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
  ...
  it.effect("merging a live first page keeps the synced tail instead of clobbering it", () =>
    Effect.gen(function* () {
      ...
      const written = yield* writeCachedT3TeamAtlassianBacklog({
        provider: "atlassian",
        accountId: "account-1",
        externalProjectId: "project-1",
        requestSelection: {},
