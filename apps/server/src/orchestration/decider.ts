      ...
      return {
        ...
        payload: {
          ...
          ...(command.activityLabel !== undefined
            ...
            : {}),
          ...(command.activityState !== undefined
            ? { activityState: command.activityState, activityStateUpdatedAt: occurredAt }
            : {}),
    ...
    case "thread.user-input.respond": {
      ...
      if (
        ...
        for (const question of payload.value.questions) {
          ...
          // Upstream rule kept: an unanswered question is rejected even when a file
          // is attached to it; only an explicit (possibly empty) answer may lean on
          // its attachments. t3team adds the multi-select array form.
          if (
            (typeof answer !== "string" && !Array.isArray(answer)) ||
            (answerText.length === 0 && questionAttachments.length === 0)
          ) {
            return yield* new OrchestrationCommandInvariantError({
              commandType: command.type,
          ...
          const attachmentLabels = questionAttachments
            ...
            .join("\n");
          replies.push(
            [`${question.question}\n${answerText}`, attachmentLabels].filter(Boolean).join("\n"),
          );
        }
