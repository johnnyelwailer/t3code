...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  it.effect("hydrates read model from projection tables and computes snapshot sequence", () =>
    Effect.gen(function* () {
      ...
      const commandReadModelWithLinks = yield* snapshotQuery.getCommandReadModel();
      assert.deepEqual(commandReadModelWithLinks.threads[0]?.pullRequests, expectedPullRequests);
      assert.deepEqual(
        commandReadModelWithLinks.threads[0]?.linkedPullRequest,
  ...
  it.effect(
    "threadExists reports true for an active thread and false for an unknown/deleted one",
    () =>
      Effect.gen(function* () {
        const snapshotQuery = yield* ProjectionSnapshotQuery;
        const sql = yield* SqlClient.SqlClient;
      ...
      `;
        ...
        VALUES
          (
            'thread-active', 'project-1', 'Active Thread',
            '{"provider":"codex","model":"gpt-5-codex"}', 'full-access', 'default',
            NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-06-01T00:00:02.000Z', '2026-06-01T00:00:03.000Z', NULL
          ),
          (
            'thread-deleted', 'project-1', 'Deleted Thread',
            '{"provider":"codex","model":"gpt-5-codex"}', 'full-access', 'default',
            NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-06-01T00:00:04.000Z', '2026-06-01T00:00:05.000Z', '2026-06-01T00:00:06.000Z'
          )
      `;

        assert.equal(yield* snapshotQuery.threadExists(ThreadId.make("thread-active")), true);
        assert.equal(yield* snapshotQuery.threadExists(ThreadId.make("thread-deleted")), false);
        assert.equal(yield* snapshotQuery.threadExists(ThreadId.make("thread-unknown")), false);
      }),
  );

  it.effect(
    "hasPendingTurnStart reports true only for active threads with a requested-but-unstarted turn",
    () =>
      Effect.gen(function* () {
        const snapshotQuery = yield* ProjectionSnapshotQuery;
        const sql = yield* SqlClient.SqlClient;
      ...
      `;
        ...
        VALUES
          (
            'thread-pending', 'project-1', 'Pending Thread',
            '{"provider":"codex","model":"gpt-5-codex"}', 'full-access', 'default',
            NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-06-01T00:00:02.000Z', '2026-06-01T00:00:03.000Z', NULL
          ),
      ...
      `;
        ...
        // GHE #343 shape: user message posted (pending start), provider has
        // not started the turn yet; a settled turn may already exist.
        yield* sql`
        INSERT INTO projection_turns (
        ...
        VALUES
          (
            'thread-pending', 'turn-settled', NULL, NULL, NULL, NULL,
            'completed', '2026-06-01T00:01:00.000Z', '2026-06-01T00:01:00.000Z',
            '2026-06-01T00:02:00.000Z', NULL, NULL, NULL, '[]'
          ),
          (
            'thread-pending', NULL, 'msg-corrective', NULL, NULL, NULL,
            'pending', '2026-06-01T00:02:00.100Z', NULL, NULL, NULL, NULL, NULL, '[]'
          ),
          (
            'thread-deleted', NULL, 'msg-deleted', NULL, NULL, NULL,
            'pending', '2026-06-01T00:02:00.200Z', NULL, NULL, NULL, NULL, NULL, '[]'
          )
      `;

        assert.equal(
          yield* snapshotQuery.hasPendingTurnStart(ThreadId.make("thread-pending")),
          true,
        );
        assert.equal(
          yield* snapshotQuery.hasPendingTurnStart(ThreadId.make("thread-deleted")),
          false,
        );
        assert.equal(
          yield* snapshotQuery.hasPendingTurnStart(ThreadId.make("thread-unknown")),
          false,
        );
      }),
  );
