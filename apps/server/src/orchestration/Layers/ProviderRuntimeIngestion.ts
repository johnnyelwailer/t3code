...
const make = Effect.gen(function* () {
  ...
  const claimJobNotificationMarker = Effect.fn("claimJobNotificationMarker")(function* (
    ...
    // Kind-filtered repository read: ingestion must never hydrate the full thread
    // detail (upstream invariant, enforced by the test harness).
    const markerActivities = yield* projectionThreadActivityRepository.listByThreadId({
      threadId,
      activityKinds: [JOB_NOTIFICATION_MARKER_KIND, JOB_NOTIFICATION_CLAIMED_KIND],
    });
    const claim = findClaimableJobNotificationMarker(
      markerActivities.map((row) => ({ id: row.activityId, kind: row.kind, payload: row.payload })),
      nowIso,
    );
    if (claim === undefined) {
      return;
