...
export type T3TeamContextTicket = {
  readonly id: string;
...
export function resourceRefToT3TeamContextTicket(input: {
  readonly projectId: string;
...
export function snapshotToT3TeamContextTicket(input: {
  readonly projectId: string;
  readonly snapshot: ResourceSnapshot;
}): T3TeamContextTicket {
  const base = resourceRefToT3TeamContextTicket({
    projectId: input.projectId,
...
export function resolveT3TeamContextTicketKey(ticket: T3TeamContextTicket): string {
  return ticket.ref.displayId || ticket.ref.id || ticket.id;
}

export function buildT3TeamContextTicketSummary(ticket: T3TeamContextTicket) {
  return [
