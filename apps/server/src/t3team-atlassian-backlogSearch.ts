...
import { appendCachedT3TeamAtlassianBacklogSyncPage } from "./t3team-atlassian-backlog-cacheReadWrite.ts";
...
import { loadSelection } from "./t3team-atlassian-backlogLivePayload.ts";
import { tryAtlassianPromise } from "./t3team-atlassian-http.ts";

const defaultBacklogSearchLimit = 50;

export type T3TeamAtlassianBacklogSearchInput = {
...
export type T3TeamAtlassianBacklogSearchResult = {
  readonly mode: "offline" | "live";
...
export function searchT3TeamAtlassianBacklog(input: T3TeamAtlassianBacklogSearchInput) {
  return input.mode === "offline" ? searchOfflineBacklogCache(input) : searchLiveBacklog(input);
}

export function searchOfflineBacklogCache(input: T3TeamAtlassianBacklogSearchInput) {
  return Effect.gen(function* () {
    const normalizedQuery = normalizeSearchQuery(input.query);
    if (!normalizedQuery) {
      return { mode: "offline", items: [] } satisfies T3TeamAtlassianBacklogSearchResult;
    }
    ...
    Effect.catch(() =>
      Effect.succeed({ mode: "offline", items: [] } satisfies T3TeamAtlassianBacklogSearchResult),
    ),
  );
}

function searchLiveBacklog(input: T3TeamAtlassianBacklogSearchInput) {
  return Effect.gen(function* () {
    const textTerm = toJqlTextTerm(input.query);
    if (!textTerm) {
      return { mode: "live", items: [] } satisfies T3TeamAtlassianBacklogSearchResult;
    }
