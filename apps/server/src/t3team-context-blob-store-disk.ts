...
import { upsertT3TeamContextBlob } from "./t3team-context-blob-store-tables.ts";
import { t3teamRandomUUID } from "./t3team-random.ts";
import { WorkspacePaths } from "./workspace/WorkspacePaths.ts";

export function ensureT3TeamContextBlobOnDisk(input: {
  ...
  return Effect.gen(function* () {
    ...
    const tempPath = `${resolved.absolutePath}.${t3teamRandomUUID()}.tmp`;
...
export function linkT3TeamContextArtifactToBlob(input: {
  readonly workspaceRoot: string;
