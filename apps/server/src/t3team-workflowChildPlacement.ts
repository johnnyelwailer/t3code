 ...
 * `appendStartChildHandoffActivities`: `t3team.handoff.created` on the CHILD (what the
 * sidebar reads when the child's own activities are loaded / via the placements route) AND
 * `t3team.handoff.started` on the PARENT (what `indexT3TeamChildParentThreads` reads so the
...
export function buildWorkflowChildPlacementCommands(input: {
  ...
  return [
    buildWorkflowChildPlacementCommand({ ...input, commandId: input.newId() }),
    {
      ...
      commandId: CommandId.make(`t3team-wf:placement-started:${input.newId()}`),
      threadId: ThreadId.make(input.parentThreadId),
      activity: {
        id: EventId.make(
          `t3team-wf-placement-started:${input.workflowRunId}:${input.childThreadId}`,
        ),
        tone: "info",
