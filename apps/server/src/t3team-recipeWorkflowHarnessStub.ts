 ...
 * `thread.turn-start-requested` domain event it dispatches the session-lifecycle + assistant
 * message commands a real `ProviderRuntimeIngestion` would (see
 * `t3team-workflowStubAgentTurn.ts`). No gateway, no provider process, no network.
 *
      ...
      yield* Effect.forkScoped(
        Stream.runForEach(orchestration.streamDomainEvents, (event) => {
          ...
          const commands = stubAgentTurnCommands({
            threadId,
            idPrefix: `harness:${turnMessageId}`,
            messages: [[reply]],
            createdAt: ISO,
          });
