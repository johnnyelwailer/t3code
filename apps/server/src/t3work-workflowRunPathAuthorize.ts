/**
 * What `t3work.orchestration.run` is allowed to EXECUTE.
 *
 * Three cases, deliberately asymmetric:
 *  1. inline `source` → persisted at `.t3work-runs/<runId>/workflow.ts` (the engine re-reads it on
 ...
 *     ({@link ./t3work-workflowRunPackAuthorize.ts}).
 */
...
import {
  ...
  confirmRunnable,
  CONTAINMENT_HINT,
} from "./t3work-workflowRunPackAuthorize.ts";
...
/**
 * Resolve what a `t3work.orchestration.run` call should execute. Inline `source` is persisted;
...
export function resolveRunWorkflowPath(input: {
  ...
  return Effect.gen(function* () {
    ...
    if (contained._tag === "Success") {
      // Workspace-local (including `.t3work-runs/<runId>/workflow.ts`): unchanged rule, re-proved
