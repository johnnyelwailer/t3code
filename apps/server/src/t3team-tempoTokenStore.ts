/**
 * Tempo API token store (see t3team-tempo.ts for the capacity integration).
 *
...
const TEMPO_TOKEN_SECRET_NAME = "t3team-tempo-token";
...
export const loadTempoToken = Effect.gen(function* () {
  const envToken = process.env["T3TEAM_TEMPO_API_TOKEN"]?.trim();
