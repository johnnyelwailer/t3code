...
import { createChildStatusEventReactor } from "./t3work-childStatusSummarizer.ts";
...
const isChildThread = (thread: {
  ...
  thread.activities.some((activity) => activity.kind === "t3work.handoff.created");

/** Observes domain events and performs a separate structured generation request. */
export const T3workChildStatusReactorLive = Layer.effectDiscard(
  Effect.gen(function* () {
      ...
      persist: async ({ threadId, status }) => {
        await Effect.runPromise(
          engine.dispatch({
            ...
            commandId: CommandId.make(`server:t3work:child-status:${t3workRandomUUID()}`),
        ...
        const meaningful =
          event.type === "thread.turn-diff-completed"
            ...
            : event.type === "thread.activity-appended" &&
                event.payload.activity.kind !== "t3work.handoff.created"
              ? {
                  threadId: event.payload.threadId,
