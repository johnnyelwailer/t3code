/* oxlint-disable t3code/no-manual-effect-runtime-in-tests -- mirrors t3work-recipeWorkflowScripts.test.ts: an async launch test bridging the Effect runtime. */
...
import { FsJournalStore } from "@t3work/sdk";
import { afterAll, describe, expect, it } from "vite-plus/test";

import {
  buildT3workHarnessScriptLog,
  readT3workHarnessScriptLog,
} from "./t3work-recipeWorkflowHarnessScriptLog.ts";
...
const workspaceRoot = NodeFS.mkdtempSync(
  NodePath.join(fixtureRoot, "t3work-script-log-workspace-"),
);
const runsRoot = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3work-script-log-runs-"));
afterAll(() => {
...
const recipeRoot = NodePath.join(workspaceRoot, ".t3work", "recipes", "script-log");
...
describe("harness scriptCalls is a real invocation log", () => {
  it("reports the journaled scripts.* dispatches, in order, and flags a declared-but-uncalled script", async () => {
    ...
    const log = await readT3workHarnessScriptLog({
  ...
  it("orders by journal seq, counts script-never markers, and ignores non-script primitives", () => {
    const log = buildT3workHarnessScriptLog({
      entries: [
