...
description: "Summarize Jira ticket or project status from t3team context without surfacing internal workspace files. Use when the user asks for ticket status, ticket context, blockers, owner, child tickets, or linked repository impact. Prefer a read-only subagent for the exploration phase."
---

# T3Team Status And Context Summary

Use this workflow when a user asks for the current state of a Jira ticket, project work item, linked repository context, or a similar t3team status question.
...
2. Ask the subagent to inspect only the relevant t3team context, setup, and reference files needed to answer the question.
