 ...
 * `t3work_recipe_list`/`t3work_recipe_validate` in the registered toolkit, and (c) get a REAL
 ...
 * binding runs the real `callT3workRecipeTool` + `makeRecipeToolHandlers` (no mock result), so the
...
const realBinding: T3workToolBinding = {
  threadId,
  listServers: () => [],
  readResource: ({ uri }) => Effect.succeed({ contents: [{ uri, text: "{}" }] }),
  callTool: ({ tool, arguments: args }): Effect.Effect<T3workToolCallResult, never> =>
    isT3workRecipeTool(tool)
      ? callT3workRecipeTool({
          tool: tool as T3workRecipeToolId,
          scopeLabel: "over /mcp",
          toolArgs: args,
          recipeTools,
        })
...
it.effect("registers t3work_recipe_list and t3work_recipe_validate in the live toolkit", () =>
  Effect.gen(function* () {
    const server = yield* McpServer.McpServer;
    const names = server.tools.map(({ tool }) => tool.name);
    expect(names).toContain("t3work_recipe_list");
    expect(names).toContain("t3work_recipe_validate");
  ...
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* server
        .callTool({ name: "t3work_recipe_validate", arguments: { source: VALID_WORKFLOW } })
        .pipe(
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      const server = yield* McpServer.McpServer;
      const result = yield* server
        .callTool({ name: "t3work_recipe_validate", arguments: { source: INVALID_WORKFLOW } })
        .pipe(
          Effect.provideService(McpInvocationContext.McpInvocationContext, {
            environmentId,
            threadId,
