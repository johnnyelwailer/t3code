...
const hostClaim = (threadId: ThreadId, providerInstanceId: ProviderInstanceId, held?: string) =>
  ({ threadId, providerInstanceId, heldAuthorizationHeader: held, authority: { _tag: "Host" } }) as const;
...
it.effect("reuses the exact bearer a thread holds, and only in the scope it holds it", () =>
  Effect.gen(function* () {
    ...
    expect((yield* registry.reuseOrReissue(hostClaim(threadId, providerInstanceId, held)))._tag).toBe(
      "Reused",
    );
