...
it.effect("re-preparing a live thread hands back the bearer its agent already holds", () =>
  serverProcess(
    Effect.gen(function* () {
      ...
      expect((yield* registry.resolve(bare(first?.config.authorizationHeader ?? "")))?.threadId).toBe(
        threadId,
      );
...
it.effect("turning agent browser access off revokes, clears and retires the thread's epoch", () =>
  serverProcess(
    Effect.gen(function* () {
      ...
      expect(yield* registry.resolve(bare(issued?.config.authorizationHeader ?? ""))).toBe(undefined);
