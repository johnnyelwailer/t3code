...
/**
 * Jira-shaped `raw` payload. `t3team-context-jira-relationships.ts` reads parent/subtask/
 * issuelink structures out of exactly these paths, so fixture relations land in
 * `t3team_context_edges` through the same extractor the live sync uses.
 */
...
function renderFixtureText(ticket: T3TeamFixtureTicket): string {
...
export function fixtureResourcePage(source: T3TeamFixtureProjectSource) {
