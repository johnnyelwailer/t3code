...
describe("context refresh HTTP routes", () => {
  it("POST context-refresh/work-item returns synced bundle metadata", async () => {
    await runRouteHttpTest(
      "t3team-context-refresh-route-",
      Effect.gen(function* () {
        const { root, project } = makeContextRefreshTestWorkspace();
        yield* Layer.build(makeRouteHttpTestLayer("t3team-context-refresh-route-"));
        ...
        expect(body.availability).toBe(T3TEAM_CONTEXT_AVAILABILITY_FULL);
  ...
  it("POST context-refresh/project returns synced bundle metadata", async () => {
    await runRouteHttpTest(
      "t3team-context-refresh-project-route-",
      Effect.gen(function* () {
        const { root, project } = makeContextRefreshTestWorkspace();
        yield* Layer.build(makeRouteHttpTestLayer("t3team-context-refresh-project-route-"));
        const httpClient = yield* HttpClient.HttpClient;
  ...
  it("POST context-refresh/work-item rejects missing required fields", async () => {
    await runRouteHttpTest(
      "t3team-context-refresh-route-err-",
      Effect.gen(function* () {
        yield* Layer.build(makeRouteHttpTestLayer("t3team-context-refresh-route-err-"));
        const httpClient = yield* HttpClient.HttpClient;
  ...
  it("POST context-refresh/work-item-slice writes a focus entrypoint", async () => {
    await runRouteHttpTest(
      "t3team-context-refresh-slice-route-",
      Effect.gen(function* () {
        const { root, project } = makeContextRefreshTestWorkspace();
        yield* Layer.build(makeRouteHttpTestLayer("t3team-context-refresh-slice-route-"));
        const httpClient = yield* HttpClient.HttpClient;
