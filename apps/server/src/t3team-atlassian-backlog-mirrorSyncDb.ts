...
export function upsertMirrorIssues(input: {
        ...
        for (const item of input.items) {
          ...
          yield* sql`
            ...
            WHERE json_extract(excluded.resource_json, '$.updatedAt') >
              COALESCE(
                json_extract(t3team_atlassian_backlog_issues.resource_json, '$.updatedAt'),
                ''
              )
          `;
        }
  ...
  }).pipe(Effect.mapError(toPersistenceSqlError("t3team.atlassianMirrorSync.upsertIssues")));
}
...
export function deleteMirrorIssuesAbsentFromWalk(input: {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.logDebug("t3team atlassian mirror reconcile pruning stale issues").pipe(
      Effect.annotateLogs({
  ...
  }).pipe(Effect.mapError(toPersistenceSqlError("t3team.atlassianMirrorSync.pruneIssues")));
}
