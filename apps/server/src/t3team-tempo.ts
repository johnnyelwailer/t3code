/**
 * Tempo capacity integration (spec docs/t3team-mvp/29-planning-space.md §10.2).
 *
 * Capacity formula (v1):
 ...
 *
 * Token: `T3TEAM_TEMPO_API_TOKEN` env var, else a persisted secret alongside
 * the Atlassian credentials (set via the /api/t3team/tempo/token route);
 * see t3team-tempoTokenStore.ts.
 */

import * as Effect from "effect/Effect";

import { providerForAccount } from "./t3team-atlassian-auth-store.ts";
import { toAtlassianError, tryAtlassianPromise } from "./t3team-atlassian-http.ts";
import {
...
import { withT3TeamIssueKeyCache } from "./t3team-tempoIssueKeyCache.ts";
import { loadTempoToken } from "./t3team-tempoTokenStore.ts";
...
export type T3TeamTempoCapacityInput = {
...
export type T3TeamTempoCapacityResult = {
  readonly configured: boolean;
  readonly from: string;
  readonly to: string;
  readonly capacities: ReadonlyArray<T3TeamTempoUserCapacity>;
};
...
function makeIssueKeyResolver(atlassianAccountId: string) {
  return Effect.gen(function* () {
    ...
    return withT3TeamIssueKeyCache(async (issueId) => {
...
export function loadT3TeamTempoCapacity(input: T3TeamTempoCapacityInput) {
  return Effect.gen(function* () {
    ...
    if (!token) {
      return {
        ...
        to: input.to,
        capacities: [],
      } satisfies T3TeamTempoCapacityResult;
    }
        ...
        tryAtlassianPromise(async () => {
          ...
          Effect.catch((error) =>
            Effect.succeed({
              ...
              workingDays: 0,
              error: error.message,
            } satisfies T3TeamTempoUserCapacity),
          ),
        ),
      ),
      { concurrency: 5 },
