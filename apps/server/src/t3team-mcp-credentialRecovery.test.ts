  ...
  serverProcess(
    Effect.gen(function* () {
      ...
      if (revoked._tag === "Revoked") {
        expect(mcpCredentialRecoveryFailureReason(revoked)).toContain("no longer has an MCP session");
      }
      ...
      expect((yield* registry.resolve(bare(live?.config.authorizationHeader ?? "")))?.threadId).toBe(
        threadId,
      );
