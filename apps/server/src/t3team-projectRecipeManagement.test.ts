...
const writeRecipe = Effect.fn("writeManagedRecipeFixture")(function* () {
  ...
  const workspaceRoot = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3team-managed-" });
  const recipePath = path.join(workspaceRoot, ".t3team/recipes/release-checklist");
