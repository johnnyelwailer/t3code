/**
 * Post-reconcile pruning of stale `t3team_atlassian_backlog_views` rows.
 * (Issue pruning after a completed walk lives in
 * `t3team-atlassian-backlog-mirrorSyncDb.ts` — `deleteMirrorIssuesAbsentFromWalk`.)
 *
 * `t3team_atlassian_backlog_views` gets one row per selection key
 * (board/sprint/filter combination) a project's backlog view has ever been
 * opened with (see `t3team-atlassian-backlog-cacheReadWrite.ts` /
 ...
 * (`t3team-atlassian-backlog-mirrorSyncWalks.ts`) can call a single small
 * function.
 */
...
export const T3TEAM_BACKLOG_VIEW_STALE_MS = 30 * 24 * 60 * 60 * 1000;
...
/**
 * Deletes `t3team_atlassian_backlog_views` rows for one (provider, account,
 ...
 */
export function pruneStaleT3TeamAtlassianBacklogViews(input: PruneStaleBacklogViewsInput) {
  return Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    const staleMs = input.staleMs ?? T3TEAM_BACKLOG_VIEW_STALE_MS;
    ...
    if (deletedRows.length > 0) {
      yield* Effect.logDebug("t3team atlassian backlog view prune removed stale rows").pipe(
        Effect.annotateLogs({
  ...
  }).pipe(Effect.mapError(toPersistenceSqlError("t3team.atlassianBacklogCache.pruneStaleViews")));
}
