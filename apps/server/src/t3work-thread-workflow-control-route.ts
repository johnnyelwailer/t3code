...
export const t3workThreadWorkflowControlRouteLayer = HttpRouter.add(
  "POST",
  ...
  Effect.gen(function* () {
    ...
    if (validationError !== null) {
      return yield* new T3workAtlassianError({ message: validationError });
    }
    ...
    if (input.action === "pause") {
      ...
      if (run.pendingCorrelationId === null) {
        return yield* new T3workAtlassianError({ message: "This workflow is not paused." });
      }
      ...
      if (run.pendingKind !== null && run.pendingThreadId !== null) {
        ...
        return yield* new T3workAtlassianError({ message: "Paused workflow has no continuation." });
      }
    } else {
      ...
      for (const childThreadId of childThreads) {
        yield* orchestration.dispatch({
          ...
          commandId: CommandId.make(`t3work-wf-stop-child:${runId}:${childThreadId}`),
    ...
    yield* orchestration.dispatch({
      ...
      commandId: CommandId.make(`t3work-wf-control:${runId}:${nowIso()}`),
      threadId: ThreadId.make(threadId),
      activity: {
        id: EventId.make(`t3work-wf-step:${runId}:run`),
        tone: "info",
    ...
    Effect.mapError((cause) => toT3workError(cause, "Failed to control workflow.")),
