...
import { T3workThreadToolContextEvictionReactorLive } from "./t3work-threadToolContextEvictionReactor.ts";
...
import {
  ...
  t3workAtlassianMyWorkRouteLayer,
...
const PtyAdapterLive = Layer.unwrap(
  Effect.gen(function* () {
    if (typeof Bun !== "undefined") {
      const BunPTY = yield* Effect.promise(() => import("./terminal/BunPtyAdapter.ts"));
      return BunPTY.layer;
    } else {
      const NodePTY = yield* Effect.promise(() => import("./terminal/NodePtyAdapter.ts"));
...
const ReactorLayerLive = Layer.empty.pipe(
  ...
  Layer.provideMerge(T3workThreadToolContextEvictionReactorLive),
...
export const makeT3workServerLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const serverApplicationLayer = Layer.mergeAll(
      HttpRouter.serve(makeT3workRoutesLayer, {
...
export const runT3workServer = Layer.launch(makeT3workServerLayer);
