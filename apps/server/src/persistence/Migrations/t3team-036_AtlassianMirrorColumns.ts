 ...
 * `t3team_atlassian_backlog_issues` table (Epic 33, Wave 2).
 *
...
export default Effect.gen(function* () {
  ...
  const tableExists = yield* sql<{ count: number }>`
    ...
    FROM sqlite_master
    WHERE type = 'table'
      AND name = 't3team_atlassian_backlog_issues'
  `.pipe(
  ...
  yield* sql`
    ALTER TABLE t3team_atlassian_backlog_issues ADD COLUMN assignee_account_id TEXT
