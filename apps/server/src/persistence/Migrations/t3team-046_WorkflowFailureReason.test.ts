...
layer("t3team-046_WorkflowFailureReason", (it) => {
  it.effect("adds nullable failure_reason + failure_step TEXT to workflow_runs", () =>
    Effect.gen(function* () {
      ...
      // A row inserted without the failure columns (pre-046 writer shape) reads back NULL.
      yield* sql`
        ...
        VALUES (
          'run-legacy-046', '/w/flow.workflow.ts', '{}', 'hash', NULL, 'proj-1',
      ...
      const rows = yield* sql<FailureRow>`
        SELECT failure_reason, failure_step FROM workflow_runs WHERE run_id = 'run-legacy-046'
      `;
