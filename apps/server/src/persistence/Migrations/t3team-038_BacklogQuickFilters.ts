/**
 * Adds the `quick_filters_json` column to `t3team_atlassian_backlog_views`
...
export default Effect.gen(function* () {
  ...
  const tableExists = yield* sql<{ count: number }>`
    ...
    FROM sqlite_master
    WHERE type = 'table'
      AND name = 't3team_atlassian_backlog_views'
  `.pipe(
  ...
  yield* sql`
    ALTER TABLE t3team_atlassian_backlog_views ADD COLUMN quick_filters_json TEXT NOT NULL DEFAULT '[]'
  ...
  yield* sql`
    ...
    WHERE selection_key NOT LIKE '%:quickFilters=%'
      AND EXISTS (
        SELECT 1 FROM t3team_atlassian_backlog_views AS newer
        WHERE newer.provider = t3team_atlassian_backlog_views.provider
          AND newer.account_id = t3team_atlassian_backlog_views.account_id
          AND newer.external_project_id = t3team_atlassian_backlog_views.external_project_id
          AND newer.selection_key = t3team_atlassian_backlog_views.selection_key || ':quickFilters=default'
      )
  `;
  yield* sql`
