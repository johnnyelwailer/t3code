...
export interface T3TeamOpenCodeConfigApplyInput {
  readonly workspaceRoot: string;
  readonly openCodeConfigRelativePath?: string;
  readonly environment?: NodeJS.ProcessEnv;
}

export interface T3TeamOpenCodeConfigApplyResult {
...
export const applyT3TeamOpenCodeMcpConfig = Effect.fn("applyT3TeamOpenCodeMcpConfig")(function* (
  input: T3TeamOpenCodeConfigApplyInput,
) {
  const fileSystem = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  ...
  } satisfies T3TeamOpenCodeConfigApplyResult;
});
