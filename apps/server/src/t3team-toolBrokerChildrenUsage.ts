...
export const T3TEAM_CHILDREN_OP_USAGE: Record<T3TeamChildOp, string> = {
  ...
  environments: `children({ op: "environments" }) — read-only discovery: the environments t3team_start_child's \`environment\` argument can target — this server's own environment (marked isDefault:true) plus the distinct cross-environment bindings already recorded on threads in this store (environments this host has targeted before, with label, bound-thread count, newest activity). Read-only, takes no arguments. Every entry states its delivery boundary: cross-environment children run on the target and stay visible here, but inter-agent messaging stays same-environment, so report-back needs a separate channel.`,
