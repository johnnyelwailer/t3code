...
import { updateCachedBacklogViewMetadata } from "./t3team-atlassian-backlog-cache.ts";
import {
...
export type T3TeamAtlassianBacklogMetadataRefreshRequest = {
  readonly account: IntegrationAccountRef;
  readonly externalProjectId: string;
  readonly selection: T3TeamBacklogSelectionInput;
};
 ...
 */
export function kickT3TeamAtlassianBacklogMetadataRefresh(
  input: T3TeamAtlassianBacklogMetadataRefreshRequest,
) {
  ...
  return Effect.gen(function* () {
    ...
    Effect.catchCause((cause) =>
      Effect.logDebug("t3team atlassian backlog metadata refresh failed", cause),
    ),
    Effect.forkDetach,
    Effect.asVoid,
  );
...
function runMetadataRefresh(input: T3TeamAtlassianBacklogMetadataRefreshRequest) {
  return Effect.gen(function* () {
