...
const workspaceRoot = NodeFS.mkdtempSync(
  NodePath.join(fixtureRoot, "t3team-recipe-scripts-workspace-"),
);
const runsRoot = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3team-recipe-scripts-"));
afterAll(() => {
...
const escapeWorkspaceRoot = NodeFS.mkdtempSync(
  NodePath.join(fixtureRoot, "t3team-recipe-scripts-escape-"),
);
afterAll(() => {
  NodeFS.rmSync(escapeWorkspaceRoot, { recursive: true, force: true });
});
const escapeRecipeRoot = NodePath.join(escapeWorkspaceRoot, ".t3team", "recipes", "escape-recipe");
...
NodeFS.writeFileSync(
  ...
  `
import { defineRecipe, defineScript, defineWorkflow } from "@t3team/sdk";
import { Schema } from "effect";
