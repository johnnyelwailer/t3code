...
export const PersistedComposerImageAttachment = Schema.Struct({
  ...
  /**
   * Absent on entries persisted before arbitrary-file attachments existed;
   * those were images, so hydration defaults to the image variant.
   */
...
export interface ComposerFileAttachment extends Omit<ChatFileAttachment, "previewUrl"> {
  ...
  /** Images carry a blob preview; arbitrary files leave this absent. */
  previewUrl?: string;
}
...
/**
 * Union of both composer attachment kinds, used at call sites that handle
 * images and arbitrary files through one pipeline.
 */
...
export interface ComposerThreadDraftState {
  prompt: string;
  /**
   * Images and arbitrary files share one slot list: the fork composer renders
   * both as chips, while the upstream `files` array additionally carries
   * reattach/upload bookkeeping for hydrated drafts.
   */
...
function composerImageDedupKey(attachment: ComposerAttachment): string {
...
function revokeObjectPreviewUrl(previewUrl: string | undefined): void {
...
export function hydrateImagesFromPersisted(
  ...
  return attachments.flatMap((attachment): ComposerAttachment[] => {
    ...
    if (attachment.type === "file") {
      // Files never render a pixel preview; keep the string contract with
      // the empty value so blob-URL bookkeeping stays a no-op for them.
      return [
        {
          type: "file" as const,
          id: attachment.id,
