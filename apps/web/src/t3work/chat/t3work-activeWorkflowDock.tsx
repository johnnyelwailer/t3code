...
import { getT3workWorkflowShapeAttachment } from "~/t3work/chat/t3work-messageShapeCard";
import type { T3workWorkflowRunProgress } from "~/t3work/chat/t3work-threadWorkflowStepProgress";

export interface T3workActiveWorkflowDockItem {
...
function summarizeStep(step: T3workWorkflowRunProgress["steps"][number]): string {
...
export function deriveT3workActiveWorkflowDockItems(
...
): ReadonlyArray<T3workActiveWorkflowDockItem> {
  const byRunId = new Map<string, T3workActiveWorkflowDockItem>();

  for (const entry of timelineEntries) {
    if (entry.kind !== "message") continue;
    const message = entry.message;
    if (!message) continue;
    const shape = getT3workWorkflowShapeAttachment(message);
...
export function T3workActiveWorkflowDock({
  items,
  className,
  onOpen,
}: {
  items: ReadonlyArray<T3workActiveWorkflowDockItem>;
  className?: string;
  onOpen: (item: T3workActiveWorkflowDockItem) => void;
}) {
  ...
  return (
    <div
      ...
      data-t3work-active-workflow-dock="true"
    >
