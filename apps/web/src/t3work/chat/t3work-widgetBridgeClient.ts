...
export const T3WORK_WIDGET_MAX_INFLIGHT_CALLS = 4;
/** Max UTF-8 byte size of the forwarded callTool request body. Mirrored server-side. */
export const T3WORK_WIDGET_MAX_ARGS_BYTES = 32 * 1024;
...
export function claimWidgetPromptSlot(widgetId: string, now = Date.now()): boolean {
  ...
  if (lastAt !== undefined && now - lastAt < T3WORK_WIDGET_PROMPT_INTERVAL_MS) return false;
...
export async function postWidgetToolCall(input: {
...
}): Promise<T3workWidgetToolCallResponse> {
  ...
  if (new TextEncoder().encode(body).byteLength > T3WORK_WIDGET_MAX_ARGS_BYTES) {
    return { ok: false, error: "Widget tool call arguments exceed the 32 KB limit." };
  }
  try {
    const response = await fetch(`${input.httpBaseUrl}/api/t3work/widget/tool-call`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body,
      signal: input.signal,
    });
    return (await response.json()) as T3workWidgetToolCallResponse;
  } catch {
