...
import { applyT3workPackAppearance } from "~/t3work/t3work-packAppearance";
 ...
 * T3WORK_STORYBOOK_PACK_DIR in the storybook main config) and rewrites
...
function PackBrandingSheet({ mode }: { mode: "light" | "dark" }) {
  ...
  useEffect(() => {
    ...
    if (appearance) applyT3workPackAppearance(appearance);
    return () => {
  ...
  if (!appearance) {
    return (
      <div className="p-8 text-sm text-muted-foreground">
        No pack theme found — run storybook with T3WORK_STORYBOOK_PACK_DIR=&lt;pack directory&gt;.
      </div>
    );
  }
  ...
  return (
    <div className="h-screen overflow-y-auto bg-background text-foreground">
      <div className="flex h-[52px] items-center gap-2 border-b border-border bg-(--app-chrome-background) px-4">
        <T3workPackBrandImage brand={appearance.brand} kind="mark" className="size-5 shrink-0" />
