...
function resolve(
  ...
  return resolveT3workComposerMenuKey({ key, items, activeItemId });
}

describe("t3workComposerMenuOptionDomId", () => {
  it("is stable and escapes item ids injectively", () => {
    expect(t3workComposerMenuOptionDomId("box", "recipe-slash-command:qa-plan")).toBe(
      t3workComposerMenuOptionDomId("box", "recipe-slash-command:qa-plan"),
    );
    expect(t3workComposerMenuOptionDomId("box", "path:file:a/b")).not.toBe(
      t3workComposerMenuOptionDomId("box", "path:file:a:b"),
    );
    expect(t3workComposerMenuOptionDomId("box", "a_b")).not.toBe(
      t3workComposerMenuOptionDomId("box", "a-b"),
    );
    expect(/^[A-Za-z0-9_-]+$/.test(t3workComposerMenuOptionDomId("box", "path:file:a/b.ts"))).toBe(
      true,
    );
  });
});

describe("resolveT3workComposerMenuKey", () => {
  ...
  it("closes on Escape even with no options", () => {
    ...
    expect(
      resolveT3workComposerMenuKey({ key: "Escape", items: [], activeItemId: null }),
    ).toEqual({ type: "close" });
  });

  it("ignores navigation and accept keys with no options", () => {
    for (const key of ["ArrowDown", "ArrowUp", "Enter", "Tab"] as const) {
      expect(resolveT3workComposerMenuKey({ key, items: [], activeItemId: null })).toEqual({
        type: "ignore",
      });
    }
  });
