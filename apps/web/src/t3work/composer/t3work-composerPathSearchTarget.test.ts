...
import {
  isT3workComposerPathSearchTargetQueryable,
...
describe("resolveT3workComposerPathSearchTarget", () => {
  it("passes cwd and query through for a path trigger so the file search actually runs", () => {
    const target = resolveT3workComposerPathSearchTarget(pathTrigger, {
      environmentId,
    ...
    expect(isT3workComposerPathSearchTargetQueryable(target)).toBe(true);
  });

  it("keeps the environment but drops cwd/query for non-path triggers", () => {
    const target = resolveT3workComposerPathSearchTarget(skillTrigger, {
      environmentId,
      cwd: "/workspace",
    });

    expect(target).toEqual({ environmentId, cwd: null, query: null });
    expect(isT3workComposerPathSearchTargetQueryable(target)).toBe(false);
  });
  ...
  it("treats a bare `@` (empty query) as not queryable, matching the chat composer", () => {
    const target = resolveT3workComposerPathSearchTarget(
      { ...pathTrigger, query: "" },
  ...
  it("returns an inert target when there is no trigger", () => {
    const target = resolveT3workComposerPathSearchTarget(null, {
      environmentId,
      cwd: "/workspace",
    });
