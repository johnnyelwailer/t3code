...
import type { T3workComposerMenuSelectionEffect } from "~/t3work/composer/t3work-composerMenuSelection";
...
const recipes: ReadonlyArray<T3workSidecarRecipeQuickStart> = [
 ...
 */
function mount() {
  const effects: T3workComposerMenuSelectionEffect[] = [];
  ...
  function Probe() {
    ...
    const commandMenu = useT3workComposerCommandMenu({
      sources: {
      ...
      buildExtraItems: (trigger) =>
        trigger.kind === "slash-command"
          ? buildT3workRecipeSlashItems({ recipes, reservedAliases: [], query: trigger.query })
          : [],
    });
    openMenu = (value) => {
      setText(value);
...
describe("kickoff composer slash menu keyboard", () => {
  ...
  it("tracks the highlight with aria-selected and aria-activedescendant", () => {
    ...
    expect(first[0]?.id).toBe(
      t3workComposerMenuOptionDomId(listboxId, "recipe-slash-command:alpha"),
    );
