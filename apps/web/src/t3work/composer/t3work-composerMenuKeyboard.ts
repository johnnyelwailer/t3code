/** Keys the t3work composer command menu claims while it is open. */
export type T3workComposerMenuKey = "ArrowDown" | "ArrowUp" | "Enter" | "Tab" | "Escape";
 ...
 */
export type T3workComposerMenuKeyAction =
 ...
 */
export function t3workComposerMenuOptionDomId(listboxId: string, itemId: string): string {
 ...
 */
export function resolveT3workComposerMenuKey(input: {
  readonly key: T3workComposerMenuKey;
  readonly items: ReadonlyArray<{ readonly id: string }>;
  readonly activeItemId: string | null;
}): T3workComposerMenuKeyAction {
  if (input.key === "Escape") {
    return { type: "close" };
  }
