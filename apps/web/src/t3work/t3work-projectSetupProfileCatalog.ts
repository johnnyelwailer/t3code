...
type Visual = Pick<T3workProjectSetupCardOption, "icon" | "accentClassName" | "iconClassName">;
...
function fromPackProfile(profile: EnvironmentSetupProfile): T3workProjectSetupCardOption {
 ...
 */
export function listT3workProjectSetupCardOptions(
  packProfiles?: readonly EnvironmentSetupProfile[],
): ReadonlyArray<T3workProjectSetupCardOption> {
  ...
  return listT3WorkProjectSetupProfiles().flatMap((profile) => {
    const visuals = BUILT_IN_VISUALS[profile.id as BundledT3WorkProfileId];
    if (!visuals) return [];
