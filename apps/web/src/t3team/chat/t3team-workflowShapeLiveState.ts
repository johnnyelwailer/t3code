...
// stale optimistic value. "failed" is deliberately excluded — a retry's optimistic "running"
// must survive until the server actually reports the re-drive, or the click would flash right
// back to the failed card it was meant to leave (GHE #344). A NEW "failed" (the re-drive settling
// again) reconciles via `controlBaselineUpdatedAt` below — status alone can't tell "still the
// pre-click failure" from "the retry failed too", but the run's `updatedAt` can.
...
export function useT3TeamWorkflowShapeLiveState(input: {
  ...
  // tells "the server hasn't caught up yet" apart from "the server settled again" (GHE #344).
  ...
  // control error surfaced via `controlError` — this just decides whether to show the button.
