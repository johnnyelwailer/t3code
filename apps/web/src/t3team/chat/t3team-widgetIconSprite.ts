...
export const T3TEAM_WIDGET_ICON_NAMES: ReadonlyArray<string> = Object.keys(ICON_NODES).toSorted();

/** `<use href="#…">` target for an icon name. */
export function t3teamWidgetIconId(name: string): string {
  return `t3w-icon-${name}`;
}

/** CSS the srcdoc reset ships so `.t3w-icon` sizes and colours itself from its text context. */
export const T3TEAM_WIDGET_ICON_CSS = [
 ...
 */
export function buildT3TeamWidgetIconSprite(): string {
  const symbols = Object.entries(ICON_NODES)
    .map(
      ([name, nodes]) =>
        `<symbol id="${t3teamWidgetIconId(name)}" viewBox="0 0 24 24" fill="none" ` +
