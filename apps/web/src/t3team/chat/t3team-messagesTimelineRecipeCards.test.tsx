...
describe("MessagesTimeline recipe cards", () => {
  ...
  it("renders an empty timeline shell", async () => {
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline {...buildT3TeamMessagesTimelineTestProps()} timelineEntries={[]} />,
    );

    expect(markup).toBeTruthy();
  }, 10000);

  it("renders workflow-card system messages from t3teamExt view attachments", async () => {
    const { MessagesTimeline } = await import("~/components/chat/MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
  ...
  it("renders generic t3team system attachments for file, image, resource, artifact, and view kinds", async () => {
    const { MessagesTimeline } = await import("~/components/chat/MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildT3TeamMessagesTimelineTestProps()}
                ...
                attachments: [
                  ...
                  },
                  {
                    ...
                    artifact: {
                      ...
                      path: ".t3team/artifacts/plan.md",
                    },
                  },
                  {
                    kind: "view",
                    miniappId: "t3team.custom-view",
    ...
    expect(markup).toContain(".t3team/artifacts/plan.md");
    expect(markup).toContain("t3team.custom-view");
  }, 10000);
