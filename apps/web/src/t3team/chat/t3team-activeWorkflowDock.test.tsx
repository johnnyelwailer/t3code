...
describe("active workflow dock", () => {
  it("keeps active and paused runs, but undocks terminal history", () => {
    ...
    const runs = new Map<string, T3TeamWorkflowRunProgress>([
    ...
    const items = deriveT3TeamActiveWorkflowDockItems(
  ...
  it("switches compactly and opens the selected workflow card", async () => {
    ...
    const container = await render(<T3TeamActiveWorkflowDock items={items} onOpen={onOpen} />);
  ...
  it("navigates the virtualized timeline to the full workflow card", async () => {
    ...
    const props = buildT3TeamMessagesTimelineTestProps();
