...
import {
  ...
  T3TEAM_WIDGET_THEME_TOKENS,
} from "~/t3team/chat/t3team-widgetSrcdoc";
import {
  buildT3TeamWidgetIconSprite,
  T3TEAM_WIDGET_ICON_CSS,
  T3TEAM_WIDGET_ICON_NAMES,
  t3teamWidgetIconId,
} from "~/t3team/chat/t3team-widgetIconSprite";
...
describe("widget theme token contract", () => {
  it("documents the semantic token vocabulary widgets may rely on", () => {
    ...
    for (const token of [
      ...
      expect(T3TEAM_WIDGET_THEME_TOKENS).toContain(token);
    }
  });
  ...
  it.each(["light", "dark"] as const)(
    ...
    (mode) => {
      applyTheme(mode);
      const themeCss = collectT3TeamWidgetThemeCss();
      ...
      const srcdoc = buildT3TeamWidgetSrcdoc({ html: authoredHtml, nonce: "n", themeCss });
  ...
  it("swaps values when the mode flips, with the same widget markup", () => {
    applyTheme("light");
    const light = collectT3TeamWidgetThemeCss();
    applyTheme("dark");
    const dark = collectT3TeamWidgetThemeCss();
...
describe("widget icon sprite", () => {
  it("ships the status icons that replace emoji, as reachable <use> targets", () => {
    const sprite = buildT3TeamWidgetIconSprite();
    for (const name of ["circle-check", "triangle-alert", "circle-x", "ban", "info", "clock"]) {
      expect(T3TEAM_WIDGET_ICON_NAMES).toContain(name);
      expect(sprite).toContain(`id="${t3teamWidgetIconId(name)}"`);
    }
    expect(t3teamWidgetIconId("check")).toBe("t3w-icon-check");
  });
  ...
  it("draws with currentColor only, so icons inherit theme-token text colour", () => {
    ...
    expect(T3TEAM_WIDGET_ICON_CSS).toContain("color: currentColor");
  });
  ...
  it("is host-injected and small, so it costs the author nothing against the 128 KB cap", () => {
    const srcdoc = buildT3TeamWidgetSrcdoc({ html: "<div/>", nonce: "n", themeCss: "" });
    expect(srcdoc).toContain('id="t3w-icon-check"');
    expect(srcdoc).toContain(".t3w-icon {");
    expect(new TextEncoder().encode(buildT3TeamWidgetIconSprite()).byteLength).toBeLessThan(
      8 * 1024,
    );
  });
  ...
  it("sizes icons at 16px/20px and wins over the reset's svg height:auto", () => {
    const srcdoc = buildT3TeamWidgetSrcdoc({ html: "<div/>", nonce: "n", themeCss: "" });
    expect(T3TEAM_WIDGET_ICON_CSS).toContain("width: 16px");
    expect(T3TEAM_WIDGET_ICON_CSS).toContain(".t3w-icon-lg { width: 20px; height: 20px; }");
  ...
  it("renders a sprite icon in a document and finds its symbol", () => {
    ...
    host.innerHTML = `${buildT3TeamWidgetIconSprite()}<svg class="t3w-icon"><use href="#${t3teamWidgetIconId("triangle-alert")}"/></svg>`;
    document.body.append(host);
    const symbol = host.querySelector(`#${t3teamWidgetIconId("triangle-alert")}`);
