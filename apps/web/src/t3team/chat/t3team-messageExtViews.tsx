...
export function getT3TeamWorkflowCardAttachment(
...
export function getT3TeamWidgetAttachments(
  message: Pick<ChatMessage, "t3teamExt">,
): ReadonlyArray<import("@t3tools/contracts").T3TeamMessageWidgetAttachment> {
  return (message.t3teamExt?.attachments ?? []).filter(
    (attachment): attachment is import("@t3tools/contracts").T3TeamMessageWidgetAttachment =>
      attachment.kind === "widget",
  );
}
...
export function T3TeamWorkflowCardBody(props: {
