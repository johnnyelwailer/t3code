...
import {
  buildT3TeamWidgetPromptTransport,
  T3TEAM_WIDGET_MAX_HEIGHT,
  T3TEAM_WIDGET_MIN_HEIGHT,
} from "~/t3team/chat/t3team-useWidgetBlockController";
import { buildT3TeamWidgetSrcdoc } from "~/t3team/chat/t3team-widgetSrcdoc";
...
describe("T3TeamWidgetBlock", () => {
  it("renders a sandboxed iframe with the widget code and nonce in srcdoc", () => {
    ...
    expect(iframe.style.height).toBe(`${T3TEAM_WIDGET_MIN_HEIGHT}px`);
  ...
  it("applies clamped resize messages from the iframe source with the right nonce", () => {
    ...
    expect(iframe.style.height).toBe(`${T3TEAM_WIDGET_MAX_HEIGHT}px`);
    expect(T3TEAM_WIDGET_MAX_HEIGHT).toBeGreaterThan(3_000);
  ...
  it("ignores messages with a wrong nonce or foreign source", () => {
    ...
    postBridgeMessage(window, { type: "t3team-widget:resize", nonce, height: 500 });
    expect(iframe.style.height).toBe(`${T3TEAM_WIDGET_MIN_HEIGHT}px`);
  });
});
...
describe("buildT3TeamWidgetSrcdoc", () => {
  it("injects theme css, transparent reset, bridge, and the raw fragment", () => {
    ...
    expect(srcdoc).toContain("t3team-widget:call-tool");
...
describe("widget prompt transport", () => {
  it("is hidden from the timeline, readable by the agent, and marked as non-workflow input", () => {
    ...
    expect(transport.t3teamExt).toEqual({
      displayText: "Approve",
      visibleToUser: false,
      visibleToAgent: true,
...
describe("widget bridge client", () => {
  it("rate-limits prompts per widget and rejects excess", async () => {
    const { claimWidgetPromptSlot, resetWidgetPromptLimiter } =
      await import("~/t3team/chat/t3team-widgetBridgeClient");
  ...
  it("only accepts callIds of the form call-<n>", async () => {
    const { isWidgetCallId } = await import("~/t3team/chat/t3team-widgetBridgeClient");
  ...
  it("rejects oversized tool-call args before hitting the network", async () => {
    const { postWidgetToolCall } = await import("~/t3team/chat/t3team-widgetBridgeClient");
