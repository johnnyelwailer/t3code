...
function buildStorybookResolveMap(baseDir: string): Map<string, string> {
  ...
  const req = NodeModule.createRequire(`${baseDir}/t3team-storybook-resolver.cjs`);
  ...
  if (map.size === 0) {
    throw new Error(
      "[t3team-storybook] Could not derive any `storybook/*` subpath resolutions from the installed storybook package.",
    );
  }
  const missing = REQUIRED_STORYBOOK_SUBPATHS.filter((subpath) => !map.has(subpath));
  if (missing.length > 0) {
    throw new Error(
      `[t3team-storybook] Storybook's builder-vite runtime subpath(s) [${missing.join(", ")}] could not be resolved from the installed storybook package's exports map. The exports shape likely changed in a Storybook upgrade; update buildStorybookResolveMap accordingly.`,
    );
  }
  return map;
}
 ...
 */
function t3teamStorybookResolver(baseDir: string): Plugin {
  const resolveMap = buildStorybookResolveMap(baseDir);
  return {
    name: "t3team-storybook-subpath-resolver",
    enforce: "pre",
    resolveId(source) {
