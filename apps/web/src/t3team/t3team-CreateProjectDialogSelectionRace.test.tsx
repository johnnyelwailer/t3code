...
// The confirm/creating steps pull in setup-profile cards, repo discovery and pack atoms that need
// a live Atom registry provider this test does not set up. Reaching "confirm" here is only about
// proving the wizard doesn't get yanked out of it — the step body's own contents are irrelevant.
...
describe("CreateProjectDialog wizard-binding selection race", () => {
  ...
  it("does not yank the confirm step backward when the background load resolves late", async () => {
    const { host, listProjectsDeferred } = await mountWizard();
    ...
    // (c) the late resolution must not move the step backward out of "confirm".
    await act(async () => {
      listProjectsDeferred.resolve(revalidatedProjects);
      await Promise.resolve();
      await Promise.resolve();
