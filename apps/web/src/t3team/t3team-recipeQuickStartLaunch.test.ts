...
import {
  ...
  buildT3TeamSelectedRecipeKickoffMessage,
  describeT3TeamSelectedRecipeQuickStart,
  getT3TeamSelectedRecipeComposerHelperText,
  getT3TeamSelectedRecipeComposerPlaceholder,
...
describe("applyT3TeamRecipeQuickStartLaunchCustomization", () => {
  it("appends launch guidance and persists structured workflow parameters", () => {
    ...
    const customized = applyT3TeamRecipeQuickStartLaunchCustomization(recipe, {
      selections: [
        {
          name: "planDepth",
          label: "Depth",
  ...
  it("returns recipe-specific composer guidance when present", () => {
    ...
    expect(getT3TeamSelectedRecipeComposerHelperText(selectedRecipe)).toBe(
      "Add any context that could change the recommendation.",
    );
    expect(getT3TeamSelectedRecipeComposerPlaceholder(selectedRecipe)).toBe(
