...
import type { AtlassianBacklogResponse } from "~/t3team/backend/t3team-types";
...
describe("project backlog cache", () => {
  it("removes only the legacy backlog payload cache entries from local storage", () => {
    const storage = new Map<string, string>([
      [
        "t3team.integration-cache.v1:atlassian:backlog:atlassian:account-1:project-1",
        "stale-backlog",
      ],
      ["t3team:project-backlog-state:v1:project-1", "selection-state"],
      ["t3team.integration-cache.v1:github:auth", "keep-me"],
    ]);
    ...
    expect(
      storage.get("t3team.integration-cache.v1:atlassian:backlog:atlassian:account-1:project-1"),
    ).toBeUndefined();
    expect(storage.get("t3team:project-backlog-state:v1:project-1")).toBe("selection-state");
    expect(storage.get("t3team.integration-cache.v1:github:auth")).toBe("keep-me");
  });
