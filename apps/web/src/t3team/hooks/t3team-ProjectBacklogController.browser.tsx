...
import type { T3TeamPollingBackend } from "~/t3team/backend/t3team-pollingBackend";
import {
  ATLASSIAN_BACKLOG_CACHE_MAX_AGE_MS,
  type BacklogSelectionInput,
} from "~/t3team/hooks/t3team-projectBacklogCache";
...
vi.mock("~/t3team/hooks/t3team-projectBacklogControllerActions", () => ({
...
vi.mock("@tanstack/react-router", async () => {
  ...
  return {
    ...
    useRouterState: ({
      ...
      select({
        location: {
          pathname: "/t3team/projects/project-1",
          search: {},
        },
      }),
  };
...
function createBackend(input: {
  ...
  readonly pollBacklog: T3TeamPollingBackend["atlassian"]["pollBacklog"];
}): BackendApi {
  const atlassian = {
...
function PersistedRouteHarness() {
  const { state } = useT3TeamPersistedRouteState<TestState, Partial<TestState>, TestRouteSearch>({
...
function PersistedRouteSetterHarness() {
  const { state, setState } = useT3TeamPersistedRouteState<
    TestState,
    Partial<TestState>,
    TestRouteSearch
  >({
...
describe("useT3TeamPersistedRouteState", () => {
  afterEach(() => {
    vi.clearAllMocks();
