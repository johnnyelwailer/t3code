...
import { loadStoredProjects } from "./t3team-projectStoreUtils";
import { resolveCanonicalProjectId, resolveStoredProjectId } from "./t3team-threadBridge";
...
export function findPinnedTicketSidebarItem(
  ...
  ticketId: string,
): Extract<T3TeamSidebarPinnedItem, { kind: "jira-work-item" }> | null {
...
export function findPinnedGitHubActivitySidebarItem(
  ...
  projectIds: ReadonlySet<string>,
  activityId: string,
): Extract<T3TeamSidebarPinnedItem, { kind: "github-activity" }> | null {
  for (const item of pinnedSidebarItems) {
    if (
