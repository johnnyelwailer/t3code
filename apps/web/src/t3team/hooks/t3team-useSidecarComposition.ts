...
} from "~/t3team/hooks/t3team-sidecarCompositionPersistence";
import {
  appendUniqueItem,
  removeItem,
  ...
  updateSectionState,
} from "~/t3team/hooks/t3team-sidecarCompositionState";
import { usePrimaryEnvironmentId } from "~/state/environments";
import { serverEnvironment } from "~/state/server";
import { useAtomCommand } from "~/state/use-atom-command";

export function useT3TeamSidecarComposition(input: {
  ...
  const updateServerSettings = useAtomCommand(serverEnvironment.updateSettings, {
    label: "t3team.sidecar.updateSettings",
  });
  const personalization = useMemo(
    () => readStoredSidecarPersonalizationFromServerSettings(serverConfig?.settings),
    [serverConfig?.settings.t3teamStoredSidecarCompositionJson],
  );
  ...
  useEffect(() => {
    ...
    return configureStoredSidecarPersonalizationPersister(async (nextJson) => {
      await updateServerSettings({
        environmentId,
        input: { patch: { t3teamStoredSidecarCompositionJson: nextJson } },
      });
    });
