...
vi.mock("~/t3team/hooks/t3team-sidecarCompositionPersistence", () => ({
  ...
  readStoredSidecarPersonalizationFromServerSettings: (settings: {
    t3teamStoredSidecarCompositionJson?: string;
  }) => {
    const raw = settings?.t3teamStoredSidecarCompositionJson;
...
type SidecarCompositionHookValue = ReturnType<typeof useT3TeamSidecarComposition>;

function renderHookValue(): SidecarCompositionHookValue {
  let captured: SidecarCompositionHookValue | undefined;

  function Probe() {
    captured = useT3TeamSidecarComposition({ bundledDefault: BUNDLED_DEFAULT });
    return null;
  }
...
describe("useT3TeamSidecarComposition", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseServerConfig.mockReturnValue({
      settings: {
        t3teamStoredSidecarCompositionJson: JSON.stringify({
          composition: {
            sections: [
              { sectionId: "quick-starts", visible: true, collapsed: false },
              { sectionId: "recent-conversations", visible: true, collapsed: false },
