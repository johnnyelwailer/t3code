...
import {
  extendT3TeamReplacementRangeForTrailingSpace,
...
function planFor(text: string, item: ComposerCommandItem) {
  ...
  return resolveT3TeamComposerMenuSelection(item, trigger, text);
}
...
describe("extendT3TeamReplacementRangeForTrailingSpace", () => {
  it("swallows an existing space only when the replacement ends with one", () => {
    expect(extendT3TeamReplacementRangeForTrailingSpace("/x here", 2, "/commit ")).toBe(3);
    expect(extendT3TeamReplacementRangeForTrailingSpace("/x here", 2, "/commit")).toBe(2);
    expect(extendT3TeamReplacementRangeForTrailingSpace("/x", 2, "/commit ")).toBe(2);
  });
});

describe("resolveT3TeamComposerMenuSelection", () => {
  ...
  it("does not double the separator when a space already follows the range", () => {
    ...
    const plan = resolveT3TeamComposerMenuSelection(
      {
        id: "provider-slash-command:claudeAgent:commit",
        type: "provider-slash-command",
        provider: claudeDriver,
...
describe("applyT3TeamComposerMenuReplacement", () => {
  it("refuses to apply when the editor text moved on", () => {
    ...
    expect(applyT3TeamComposerMenuReplacement("something else", plan!.replacement)).toBeNull();
  });
});
