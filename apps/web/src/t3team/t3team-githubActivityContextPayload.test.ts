...
import { buildJiraTicketEntryPoint } from "~/t3team/t3team-contextCachePaths";
...
describe("buildGitHubActivityContextBundle", () => {
  it("includes linked ticket entrypoints and merges linked ticket files", () => {
    ...
    expect(bundle.fileReferences).toEqual([
      {
        ...
        relativePath:
          ".t3team/context/github/project-alpha/example-project-alpha/pr-42/entrypoint.json",
      },
    ...
    expect(JSON.parse(entryPoint?.contents ?? "{}")).toMatchObject({
      ...
      paths: {
        linkedWorkItem:
          ".t3team/context/github/project-alpha/example-project-alpha/pr-42/linked-work-item/context.json",
      },
    });
  });
  ...
  it("writes a rich pull request artifact package when full PR context is available", () => {
      ...
      expect.arrayContaining([
        {
          label: "PR overview",
          relativePath:
            ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/overview.md",
        },
        {
          label: "PR diff",
          relativePath:
            ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/diff.diff",
        },
        {
          label: "File snapshots index",
          relativePath:
            ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/snapshots/index.json",
        },
      ]),
    );
    ...
    expect(
      bundle.files.some(
        (file) =>
          file.relativePath ===
          ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/snapshots/head/src/context.ts",
      ),
    ).toBe(true);
    ...
    expect(JSON.parse(entryPoint?.contents ?? "{}")).toMatchObject({
      paths: {
        pullRequest: {
          overview:
            ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/overview.md",
          diff: ".t3team/context/github/project-alpha/example-project-alpha/pr-42/pull-request/diff.diff",
        },
      },
    });
  });
