...
import { normalizeRelationshipKey } from "~/t3team/t3team-ticketRelationshipKeys";
import type { ProjectTicket } from "~/t3team/t3team-types";

const CONTEXT_ROOT_PREFIX = `${T3TEAM_PROJECT_CONTEXT_ROOT}/`;
...
export function assertContextRelativePathInProjectWorkspace(input: {
  ...
  if (
    ...
    throw new Error("Context paths must stay under the managed .t3team/context tree.");
  }

  const jiraPrefix = `${T3TEAM_PROJECT_CONTEXT_ROOT}/jira/${sanitizePathSegment(input.projectId)}/`;
  if (normalizedPath.startsWith(`${T3TEAM_PROJECT_CONTEXT_ROOT}/jira/`)) {
...
export function readContextAvailabilityFromEntryPoint(
  entryPoint: unknown,
): T3TeamContextAvailability | undefined {
  if (!entryPoint || typeof entryPoint !== "object") {
    return undefined;
  }
  const availability = (entryPoint as { availability?: unknown }).availability;
  return availability === T3TEAM_CONTEXT_AVAILABILITY_FULL ? availability : undefined;
}
