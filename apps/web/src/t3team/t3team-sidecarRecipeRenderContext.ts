...
function resolveRecipeSurface(
  input: Pick<T3TeamSidecarRecipeInput, "surface" | "dashboardMode">,
): RecipeSurface {
...
function hasAttachedWorkitem(
  attachments: ReadonlyArray<T3TeamContextAttachment> | undefined,
): boolean {
...
function hasExplicitWorkitemContext(input: T3TeamSidecarRecipeInput): boolean {
  return Boolean(
...
export function buildRecipeRenderContext(
  ...
  profile: ReturnType<typeof getT3TeamProfile>,
...
export function buildProjectRecipeDiscoveryRequest(
  input: T3TeamSidecarRecipeInput & { readonly workspaceRoot: string },
