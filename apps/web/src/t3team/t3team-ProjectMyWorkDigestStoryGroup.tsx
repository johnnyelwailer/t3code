...
export function DigestStoryGroupHeader({
  ...
  return (
    <div
      ...
      onClick={() => window.open(story.ref.url, "_blank", "noopener")}
    >
    ...
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-border/50 bg-muted/20 px-3 py-1.5">
      {children.map((child) => (
        <a
          key={child.id}
          href={child.ref.url}
          className="inline-flex max-w-full items-center gap-1 rounded-md bg-background/70 px-1.5 py-0.5 text-[11px] text-muted-foreground ring-1 ring-border/50 hover:text-foreground hover:ring-border"
          onClick={(e) => e.stopPropagation()}
        >
          <JiraIssueTypeIcon issueType={child.issueType} className="size-3 shrink-0" />
          <span className="shrink-0 font-mono text-[10px]">{child.ref.displayId}</span>
          <span className="truncate">{child.ref.title}</span>
          <span className="shrink-0 text-[10px] text-foreground/50">{child.status}</span>
          {child.assignee ? (
...
export function DigestStoryGroup({
  ...
  return (
    <T3SurfacePanel tone="muted" className="overflow-hidden">
      {story ? <DigestStoryGroupHeader story={story} graph={graph} nowMs={nowMs} /> : null}
      <div className="divide-y divide-border/50">{children}</div>
      {story ? (
        <DigestOtherChildren children={otherChildren ?? []} viewerName={viewerName} />
      ) : null}
    </T3SurfacePanel>
  );
}
