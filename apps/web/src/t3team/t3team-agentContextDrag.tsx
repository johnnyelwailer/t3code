...
const T3TEAM_AGENT_CONTEXT_DRAG_TYPE = "application/x-t3team-agent-context";
...
function readDragRecord(dataTransfer: DataTransfer | null): AgentContextDragRecord | null {
  const dragId = dataTransfer?.getData(T3TEAM_AGENT_CONTEXT_DRAG_TYPE) || activeDragId;
  if (!dragId) {
    return null;
  }
...
export function useT3TeamAgentContextDrag(input: {
  ...
  const onDragStart = useCallback(
    (event: React.DragEvent<HTMLElement>) => {
      ...
      event.dataTransfer.setData(T3TEAM_AGENT_CONTEXT_DRAG_TYPE, dragId);
...
export function useT3TeamAgentContextDropTarget(input: {
...
export function T3TeamAgentContextDropOverlay({
  active,
  label,
  className,
}: {
