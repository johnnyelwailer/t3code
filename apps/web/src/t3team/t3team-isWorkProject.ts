 ...
 * GitHub, or a t3team-managed workspace. A loose `"local"` source is just a folder the user pointed
 * t3team at (their own repo / an arbitrary directory).
 *
 * This is the gate for project-setup scaffolding. Only work projects may receive the
 * agent-instruction files (`AGENTS.md` / `CLAUDE.md`), the `.t3team` setup tree, and the managed
