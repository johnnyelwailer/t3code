...
const makeWorkspacePath = (title: string): string => {
  ...
  const base =
    platform === "darwin"
      ? `${home}/Library/Application Support/T3 Code/t3team/projects`
      : platform === "win32"
        ? `${home}/AppData/Roaming/T3 Code/t3team/projects`
        : platform === "browser"
          ? `${home}/.t3code/t3team/projects`
          : `${home}/.config/T3 Code/t3team/projects`;
  return `${base}/${normalizeWorkspaceDirectoryName(title)}`;
};

export const t3teamCreateProject = (input: CreateProjectInput) =>
  Effect.gen(function* () {
