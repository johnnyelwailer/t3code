...
} from "~/t3team/hooks/t3team-useWorkItemContextSyncQueue";
...
describe("createTicketKickoffThread", () => {
  ...
  it("pins the work item when a kickoff thread is created", async () => {
    ...
    expect(useT3TeamPinnedSidebarStore.getState().items).toEqual([
      expect.objectContaining({
        kind: "jira-work-item",
        projectId: "project-1",
        ticketId: "ticket-9",
    ...
    expect(
      useT3TeamSidebarNavPreferencesStore.getState().preferencesByProjectId["project-1"],
    ).toEqual({
      hiddenItemIds: [],
      orderedItemIds: [
        buildTicketSidebarPinnedItemId({
