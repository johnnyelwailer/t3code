...
describe("snapshot coordinates after compression", () => {
  ...
  it.each(["stash", "delivery"])(
    ...
    async (path) => {
      ...
      const restoredSource = restored?.source;
      if (!restoredSource || !("kind" in restoredSource) || restoredSource.kind !== "snap-shot") {
        throw new Error("expected the persisted image to restore as a snap-shot source");
      }
      expect(restoredSource.accessibility).toMatchObject({
