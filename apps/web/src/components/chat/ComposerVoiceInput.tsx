...
export function ComposerVoiceInput({
  ...
  const startRecording = useCallback(() => {
    ...
    const Ctor =
      (window.SpeechRecognition ||
        (window as any).webkitSpeechRecognition) as new () => SpeechRecognition;
    if (!Ctor) {
      setState("denied");
      return;
    }
    ...
    recognition.onend = () => {
      if (state === "recording" && recognitionRef.current === recognition) {
        try { recognition.start(); } catch { stopRecording(false); }
      }
    };
  ...
  // Stop
  const stopRecording = useCallback((cancelled: boolean, autoSubmit?: boolean) => {
    setState("idle");
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    if (audioCtxRef.current) { audioCtxRef.current.close(); audioCtxRef.current = null; }
    displayHeightsRef.current = new Array(BAR_COUNT).fill(4);
    barElsRef.current.forEach((el) => { if (el) el.style.height = "4px"; });
  ...
  const startWaveform = useCallback(() => {
    ...
    if (!audioCtx) { startCssBars(); return; }
      ...
      const draw = () => {
        ...
        for (let i = 0; i < BAR_COUNT; i++) {
          ...
          for (let j = 0; j < sliceLen; j++) {
            const v = Math.abs(buf[i * sliceLen + j] - 128) / 128;
            if (v > peak) peak = v;
            totalEnergy += v;
          }
          const target = Math.max(4, Math.round(22 * Math.pow(peak, 0.6)));
          displayHeightsRef.current[i] += (target - displayHeightsRef.current[i]) * LERP;
          const el = barElsRef.current[i];
          if (el) el.style.height = Math.round(displayHeightsRef.current[i]) + "px";
        }
  ...
  const startCssBars = useCallback(() => {
    const anim = () => {
      if (state !== "recording") return;
      for (let i = 0; i < BAR_COUNT; i++) {
        const target = Math.max(4, Math.round(22 * (Math.abs(Math.sin(Date.now() / 400 + i * 0.9)) * 0.7 + 0.15)));
        displayHeightsRef.current[i] += (target - displayHeightsRef.current[i]) * 0.1;
        const el = barElsRef.current[i];
        if (el) el.style.height = Math.round(displayHeightsRef.current[i]) + "px";
      }
  ...
  const switchLang = useCallback((code: string) => {
    ...
    setTimeout(() => {
      if (state !== "recording") return;
      try {
        const Ctor = (window.SpeechRecognition || (window as any).webkitSpeechRecognition) as new () => SpeechRecognition;
        const rec = new Ctor();
        rec.lang = code;
        rec.continuous = true;
        ...
        rec.onend = () => {
          if (state === "recording" && recognitionRef.current === rec) {
            try { rec.start(); } catch { stopRecording(false); }
          }
        };
        recognitionRef.current = rec;
        rec.start();
      } catch { stopRecording(false); }
    }, 150);
  ...
  return (
    <div className={cn("flex items-center gap-2", className)} data-voice-input data-state={state}>
      {/* Lang group */}
      {isRecording && (
        <div className="flex items-center gap-0.5">
          {LANG_OPTIONS.map((opt) => (
            <button
              ...
              onClick={(e) => { e.stopPropagation(); switchLang(opt.code); }}
            >
              {opt.label}
            </button>
          ))}
      ...
      >
        <svg
          className={cn("transition-opacity", isRecording && "opacity-0")}
        ...
        <div className="fixed inset-x-0 bottom-0 z-50 flex items-center justify-center gap-3 border-t border-border/50 bg-background px-6 py-3.5">
          <div className="relative flex items-center">
            ...
            <button
              ...
              onClick={(e) => { e.stopPropagation(); setStopMenuOpen(!stopMenuOpen); }}
                ...
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      stopMode === mode ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent/50"
                    )}
                    onClick={(e) => { e.stopPropagation(); setStopMode(mode); setStopMenuOpen(false); }}
                  >
                    {label}
                  </button>
                ))}
          ...
          <span className="text-sm font-medium">
            {stopMode === "manual" ? "Tap to interrupt" : stopMode === "auto3" ? "Auto-send (3s pause)" : "Auto-send (5s pause)"}
          </span>
        </div>
      )}
    </div>
