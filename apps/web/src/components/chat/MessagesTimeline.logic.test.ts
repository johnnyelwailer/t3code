...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps a promptless restart in one active visual response", () => {
    ...
    // GHE #236: the working row is pinned to the bottom of the active visual
    // response, so it lands AFTER the restart's old work entry.
  ...
  it("renders a single completed tool call directly", () => {
    ...
    // GHE #236: the working row is pinned to the bottom, after the content it
    // frames — never at the top of the visual response.
  ...
  it.each([
    ...
    (toolLifecycleStatus, _expectedLiveRow) => {
      ...
      // Upstream's live-activity row surfaces the in-flight task entry by its
      // own id; a FAILED task stops rendering the live row entirely.
