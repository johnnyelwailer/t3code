    ...
    <ComposerTerminalContextActionsContext value={terminalContextActions}>
      <ComposerCitationCommentContext value={citationCommentActions}>
        {/* Fall back to `inherit`, not var(--font-sans): the shell around the composer may set its
            own body face (the t3team shell does), and the composer should follow it unless the
            user picked an explicit composer font in Settings -> Appearance. */}
        <div
          className={cn(
            "relative [font-family:var(--font-composer,inherit)] [font-size:var(--font-size-prompt,0.875rem)] [@media(max-width:39.999rem)_and_(pointer:coarse)]:[font-size:max(var(--font-size-prompt,1rem),16px)]",
            containerClassName,
          )}
        >
          <PlainTextPlugin
