...
for (const [sourceName, targetName] of [
  ["t3team-storybook-main.ts", "main.ts"],
  ["t3team-storybook-preview.ts", "preview.ts"],
]) {
  NodeFS.writeFileSync(
    NodePath.join(configDir, targetName),
    NodeFS.readFileSync(NodePath.join(webRoot, "src/t3team/storybook", sourceName), "utf8"),
  );
}
...
try {
  ...
  if (mode === "build") {
    ...
    const preferredPort = Number.parseInt(process.env.T3TEAM_STORYBOOK_PORT ?? "6006", 10);
