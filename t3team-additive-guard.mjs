...
function main() {
  ...
  if (violations.length > 0) {
    console.error("t3team additive guard failed\n");
  ...
  if (warnings.length > 0) {
    console.warn("t3team additive guard warnings\n");
    for (const warning of warnings) {
  ...
  console.log(
    `t3team additive guard passed (${candidates.size} changed files checked against ${baseRef}).`,
  );
}
