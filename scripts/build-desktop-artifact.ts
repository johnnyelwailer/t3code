...
export function windowsVswherePrerequisiteScript(arch: typeof BuildArch.Type): string {
  ...
  const libraryArch = arch === "arm64" ? "arm64" : "x64";
  return [
    "$vswhere = Join-Path ${env:ProgramFiles(x86)} 'Microsoft Visual Studio\\Installer\\vswhere.exe'",
    "if (!(Test-Path $vswhere)) { exit 1 }",
    `$install = & $vswhere -latest -products * -requires ${toolsComponent} -property installationPath`,
    "if (!$install) { exit 1 }",
    "$toolsetsRoot = Join-Path $install 'VC\\Tools\\MSVC'",
    "$toolset = Get-ChildItem $toolsetsRoot -Directory -ErrorAction SilentlyContinue | Sort-Object Name -Descending | Select-Object -First 1",
    `if (!$toolset -or !(Test-Path (Join-Path $toolset.FullName 'lib\\spectre\\${libraryArch}'))) { exit 1 }`,
