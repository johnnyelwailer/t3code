#!/usr/bin/env node
/**
 * Live context-refresh E2E against a running t3team server (mock Jira provider).
 *
 * Usage:
...
import { extractSessionCookie, fetchJson } from "./t3team-timer-e2e-live-lib.mjs";
...
function findSqliteDb() {
  ...
  for (const file of candidates) {
    ...
    try {
      ...
      if (tables.includes("t3team_context_refresh_jobs")) return file;
    } catch {
      // try next
    }
  }
...
function queryContextRefreshJobStatus(dbPath, jobId) {
  const row = NodeChildProcess.execSync(
    `sqlite3 "${dbPath}" "SELECT status FROM t3team_context_refresh_jobs WHERE job_id='${jobId}';"`,
    { encoding: "utf8" },
  ).trim();
  return row || null;
}
