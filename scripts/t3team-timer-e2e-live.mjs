#!/usr/bin/env node
/**
 * Live sleeping-routine E2E against a running t3team server.
 *
 * Usage:
 *   node scripts/t3team-timer-e2e-workspace.mjs   # prints WORKSPACE_ROOT
 *   T3TEAM_PAIRING_TOKEN=<token> node scripts/t3team-timer-e2e-live.mjs
 */
...
import {
  ...
  queryWorkflowRun,
  sleep,
} from "./t3team-timer-e2e-live-lib.mjs";

const repoRoot = NodePath.join(NodePath.dirname(NodeURL.fileURLToPath(import.meta.url)), "..");
const baseUrl = process.env.T3TEAM_SERVER_URL ?? "http://localhost:3773";
const pairingToken = process.env.T3TEAM_PAIRING_TOKEN?.trim();
const delayMs = Number(process.env.T3TEAM_DELAY_MS ?? "2000");

if (!pairingToken) {
  console.error("T3TEAM_PAIRING_TOKEN is required (see server startup pairingUrl).");
  process.exit(1);
}

const workspaceRoot =
  process.env.T3TEAM_WORKSPACE_ROOT?.trim() ||
  NodeChildProcess.execSync("node scripts/t3team-timer-e2e-workspace.mjs", {
    cwd: repoRoot,
    encoding: "utf8",
  })
    .split("\n")
...
const workflowPath = NodePath.join(
  workspaceRoot,
  ".t3team/recipes/example-timer/example-timer.workflow.ts",
);
...
async function main() {
  ...
  const { body: launch } = await fetchJson(baseUrl, "/api/t3team/thread/recipe-workflow/launch", {
    method: "POST",
    headers: { "content-type": "application/json", cookie },
    body: JSON.stringify({
      threadId,
