...
const CACHE_FILE = NodePath.join(".git", "hooks", "t3team-additive-fast-hook-cache.json");
...
function loadConfig(cwd) {
  const configPath = NodePath.join(cwd, ".t3team-additive-guard.json");
...
function buildSystemMessage({ violations, warnings, scanned }) {
  const lines = [
    `t3team fast additive hook checked ${scanned} new file${scanned === 1 ? "" : "s"}.`,
  ];
