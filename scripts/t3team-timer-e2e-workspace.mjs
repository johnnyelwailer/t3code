 ...
 * Usage: node scripts/t3team-timer-e2e-workspace.mjs
...
const fixtureWorkflow = NodePath.join(
  repoRoot,
  "apps/server/__fixtures__/t3team-exampleTimer.workflow.ts",
);
const fixturesDir = NodePath.join(repoRoot, "apps/server/__fixtures__");
const workspaceRoot = NodeFS.mkdtempSync(NodePath.join(fixturesDir, "t3team-timer-e2e-"));
const recipeRoot = NodePath.join(workspaceRoot, ".t3team", "recipes", "example-timer");
