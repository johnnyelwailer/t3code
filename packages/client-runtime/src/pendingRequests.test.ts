...
describe("pending questions", () => {
  ...
  it("maps the question's context through to the panel question (t3team)", () => {
    const question = {
      id: "ctx-1",
      header: "Ship order",
  ...
  it("drops empty-string context so the panel does not render a blank strip (t3team)", () => {
    const activities = [
      makeActivity({
    ...
    const parsed = derivePendingRequests(activities).userInputs[0]?.questions?.[0];
