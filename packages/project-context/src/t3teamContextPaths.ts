...
export function buildProjectContextCacheRoot(_projectId: string): string {
  return T3TEAM_PROJECT_CONTEXT_ROOT;
}
...
export function buildJiraTicketCacheRoot(projectId: string, ticketKey: string): string {
  return `${T3TEAM_PROJECT_CONTEXT_ROOT}/jira/${sanitizePathSegment(projectId)}/items/${sanitizePathSegment(ticketKey)}`;
}
...
export function buildGitHubActivityCacheRoot(input: {
  ...
  return `${T3TEAM_PROJECT_CONTEXT_ROOT}/github/${sanitizePathSegment(input.projectId)}/${sanitizePathSegment(
    input.repository,
