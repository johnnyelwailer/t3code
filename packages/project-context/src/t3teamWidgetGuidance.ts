 ...
 * sandbox's CSP.
 *
 * Surfaces that need this exact text and must not restate it:
 * - the `t3team.widget.show` MCP tool's `widget_code` input schema, both in the
 *   live toolkit (`apps/server/src/mcp/toolkits/t3team/tools.ts`) and the catalog
 *   snapshot ({@link ./t3teamToolCatalogImplemented.ts}), locked by
 *   `t3teamWidgetGuidance.test.ts` and the live-schema sync assertions in
 ...
 */
/**
 * The tool-level description the `t3team.widget.show` surfaces must advertise. It carries the
 * rules an agent needs BEFORE it writes `widget_code` — theme variables for every color,
 * fluid/compact layout, transparent background, sprite icons — because the full
 * {@link T3TEAM_WIDGET_AUTHORING_GUIDANCE} text only reaches the model as the `widget_code`
 * property description, and some surfaces trim property descriptions. The catalog snapshot and
 * the live MCP toolkit must both use this constant so the model-facing text cannot drift.
 */
export const T3TEAM_WIDGET_SHOW_TOOL_DESCRIPTION =
