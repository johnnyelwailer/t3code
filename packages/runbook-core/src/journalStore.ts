...
/**
 * The filesystem implementation stores `<runs-root>/<run-id>/journal.jsonl` plus a sibling
 * `runMeta.json`. Each append is its own fsync-durable line, so torn-tail recovery (in
 * {@link readJournalEntries}) still applies on this backend.
 */
