...
} from "./t3team-packs.index.ts";
import type { LoadedWorkspacePack, WorkspacePackScope } from "./t3team-packs.index.ts";
...
describe("local discovery", () => {
  it("loads valid child packs and reports invalid children", async () => {
    const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "t3team-packs-"));
    roots.push(root);
    await Promise.all([
