 ...
 *
 * The on-disk shape is `.t3team-runs/<run-id>/journal.jsonl` (one JSONL line per journaled
 ...
 * `.t3team-runs/<run-id>/journal.jsonl` is the MVP on-disk shape the spec documents.
 */
...
// `fs`/`path` via NodeModule.createRequire — see the note in t3team-sdk.journalWriter.ts.
...
/**
 * Create `.t3team-runs/<run-id>/` and an empty `journal.jsonl` if they do not exist.
