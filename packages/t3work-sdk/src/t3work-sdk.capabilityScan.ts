 ...
 *   • `"script"`   → `scripts.*` is bound at all      (t3work-sdk.bodyRunner.ts)
 *   • `"user"`     → `askUser` / `notifyUser` / `showWidget` on a thread
 *                                                     (t3work-sdk.threadPrimitives.ts)
 *   • `"schedule"` → `waitUntil`                       (t3work-sdk.schedulePrimitive.ts)
 *   • tool group   → `tools.<id>` at its call site     (t3work-sdk.capabilityGating.ts)
 *
...
import { finding, memberChain, type WorkflowAuditFinding } from "./t3work-sdk.staticAuditTypes.ts";
