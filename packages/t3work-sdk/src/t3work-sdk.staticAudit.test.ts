...
import { auditWorkflowSourceStatic } from "./t3work-sdk.staticAudit.ts";
...
// the same). Without this, `tools.t3work.thread.rename` is an unknown id and stays silent.
import "./tools/t3work-sdk.t3work.ts";
...
describe("determinism scan", () => {
  const findings = auditFixture("t3work-sdk.determinismBad.workflow.ts");
  ...
  it("leaves the journaled ambient globals alone (Epic 25 rule 1)", () => {
    const clean = auditFixture("t3work-sdk.determinismOk.workflow.ts");
    expect(rules(clean)).toEqual([]);
  });
});

describe("static capability check", () => {
  const findings = auditFixture("t3work-sdk.capabilityBad.workflow.ts");
  ...
  it("is clean once the capabilities are declared", () => {
    expect(rules(auditFixture("t3work-sdk.capabilityOk.workflow.ts"))).toEqual([]);
  });

  it("honours an explicitly supplied declared set over meta", () => {
    const withUser = auditFixture(
      "t3work-sdk.capabilityBad.workflow.ts",
  ...
  it("flags a tools.* call whose resolved group is undeclared", () => {
    const source = {
      ...
      sourceText: [
        "export const meta = { name: 'inline', capabilities: [] };",
        "const out = await tools.t3work.thread.rename({ title: 'x' });",
        "return { out };",
      ].join("\n"),
    };
    const findings = auditWorkflowSourceStatic(source, { declared: new Set() });
    expect(findings).toHaveLength(1);
    expect(findings[0]?.message).toContain("'t3work.thread.write'");
    expect(findings[0]?.construct).toBe("tools.t3work.thread.rename");
  });
