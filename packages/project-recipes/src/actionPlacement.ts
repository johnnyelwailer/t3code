/* oxlint-disable t3code/no-inline-schema-compile -- Matches the sibling sidecarSection.ts placement schema style. */
/**
 ...
 * inputs), bound to a recipe id and a set of surfaces. Today bundled recipes carry that view
 * as a raw `actionViewTemplate` string with no validation
 * ({@link ../../t3work-skill-packs/src/recipeActionViews.ts}) and the web layer lifts it into
 * `quickStart.actionView.source`
 * ({@link ../../../apps/web/src/t3work/t3work-sidecarRecipes.ts}) — `defineAction` gives that
