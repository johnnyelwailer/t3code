...
4. **Resolve seams**: ours-side = t3work extension props/imports; upstream side may have restructured the same region — re-apply the t3work extension points onto the new upstream shape rather than picking a side wholesale.
5. **Guard**: `node t3work-additive-guard.mjs` must pass (or list only pre-existing violations — diff against its pre-merge output).
