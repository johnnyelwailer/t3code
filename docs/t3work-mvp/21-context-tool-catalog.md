...
Provider MCP wiring (Codex path):

- Web syncs `T3workTurnToolContext` to `POST /api/t3work/thread/tool-context`.
- `ProviderService.prepareMcpSession` and the tool-context route call
  `maybeBindT3workBrokerForProviderThread`, which reads the store, resolves
  `allowedToolGroups` (ticket threads default to `artifact.rw`), and stores the binding in
  `t3work-toolBrokerSessionRegistry`.
- `/mcp` registers implemented t3work tools and routes `tools/call` through the bound broker.
  MCP credentials include the `t3work` capability when a binding exists.
- Codex reloads the MCP catalog before each turn via `config/mcpServer/reload` when the
  t3-code MCP server is attached.
