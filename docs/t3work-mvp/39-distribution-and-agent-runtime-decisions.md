...
This note captures three architecture questions for the public t3work core and the private Nexi distribution.
...
OpenCode is useful as an existing integration, but it is too opinionated for the private Nexplore agent path. t3work already owns the product-level orchestration:
...
3. Extend `@t3work/pack-api` with the neutral `AgentDriver` and `AgentSession` contracts.
