...
`apps/web/src/t3work/chat/t3work-widgetTheming.test.tsx`.

**Theme tokens.** `collectT3workWidgetThemeCss()` snapshots the live document's custom
...
The table is the *documented* contract, mirrored by `T3WORK_WIDGET_THEME_TOKENS`. The
...
**Icons.** The host injects an inline `<symbol>` sprite (`t3work-widgetIconSprite.ts`) into
