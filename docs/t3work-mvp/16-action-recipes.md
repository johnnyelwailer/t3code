...
| **Tools**    | The single capability surface — the verbs that read or mutate `t3work` and external state. The _same_ surface is consumed by agent turns, orchestration steps, and views, scoped by `allowedToolGroups`. See [Epic 21](./21-context-tool-catalog.md).                                                                                     | `T3workToolBroker`         |
...
> (`apps/server/src/t3work-workflowEngine*.ts` — Broker/Launch/Reactor/Registry/Rehydrate/Resume/
...
> `apps/server/src/t3work-thread-recipe-workflow-routes.ts`,
> `apps/server/src/t3work-workflowEngineLaunch.ts`, and
...
- Thread bootstrap and the special recipe launch message live in the t3 adapter / server
  ...
  [t3work-workflowEngineBroker.ts](apps/server/src/t3work-workflowEngineBroker.ts),
  [t3work-workflowEngineLaunch.ts](apps/server/src/t3work-workflowEngineLaunch.ts),
  [t3work-workflowEngineReactor.ts](apps/server/src/t3work-workflowEngineReactor.ts),
  [t3work-workflowEngineRegistry.ts](apps/server/src/t3work-workflowEngineRegistry.ts),
  [t3work-workflowEngineRehydrate.ts](apps/server/src/t3work-workflowEngineRehydrate.ts),
  [t3work-workflowEngineResume.ts](apps/server/src/t3work-workflowEngineResume.ts), and
  [t3work-workflowEngineDurability.ts](apps/server/src/t3work-workflowEngineDurability.ts) —
  plus [packages/project-recipes/src/runtime.ts](packages/project-recipes/src/runtime.ts). The
  earlier single-file `t3work-recipeWorkflowRuntime.ts` no longer exists.
