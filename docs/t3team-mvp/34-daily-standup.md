...
2. Scheduled pre-standup digest (§6.1) deep-links into the surface with the day's window
   pre-selected.
3. Agent tool `t3team.project.open_dashboard_mode` gains the `standup` target.
...
standup-local preferences persist (backend SQLite, migration `t3team-036_Standup`):

```sql
t3team_standup_prefs   (project_id, board_key, order_strategy, manual_order JSON NULL,
                        window_kind, updated_at)
t3team_standup_runs    (id, project_id, board_key, sprint_id, started_at, ended_at,
                        order_strategy, notes JSON NULL)   -- one row per facilitated run
t3team_standup_notes   (run_id, account_id, talking_points TEXT NULL,
...
- Each person's panel is the focus; facilitator captures talking points + action items
  (stored in `t3team_standup_notes`).
...
```text
# read (safe)
t3team.standup.read_board            # ordered roster + per-person panels + graph
t3team.standup.read_person
t3team.standup.read_graph            # the dependency edges with evidence

# view-state (safe)
t3team.standup.open_view             # open standup mode, optional sprint/window
t3team.standup.set_order             # switch strategy or apply a manual order

# local + draft mutations (reviewed)
t3team.standup.start_run / end_run
t3team.standup.capture_note          # {accountId, talkingPoints?, actionItems?}
t3team.standup.draft_followup        # → reviewable Jira/GitHub mutation
```
...
6. **P6 — Composable view**: register as a `project.navView` and express the surface
   through `@t3team/blocks` (new blocks: `StandupRoster`, `StandupPersonPanel`,
