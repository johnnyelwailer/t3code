...
Give agents in `t3team` a richer expressive surface than plain markdown in chat
...
primitive (mounted via `t3teamExt.view` on a message). What is missing is the
...
| **T2: structured view**     | `Message.t3teamExt.view` | Heavy, stateful, interactive components               | **T2a:** registered miniapp invocation (Epic 19) · **T2b:** on-the-fly composer subagent that authors a new view in seconds |
...
- **Carried structurally.** Invoked via `t3teamExt.view` on the message envelope; props
...
`t3teamExt.view` envelope and the same renderer.
...
_Reuse existing infrastructure._ t3team already has a workspace-level
...
- **`scope: "project"`** — module lands in the project-local miniapp source location as a
  normal Epic 19 miniapp. Current examples spell this as `.t3team/miniapps/<view-id>/`,
...
- **`scope: "home"`** — user-global, lands in the home workspace's
  `.t3team/miniapps/`. Reserved for views the user explicitly promotes.
...
T2 components do not stream by syntax. The `t3teamExt.view` payload carries one
...
`compose_view` tool is contributed by the t3team runtime and gated by the active
...
Message
  ...
  └─ t3teamExt.displayText?           ← user-facing text when provider text includes hidden context
  └─ t3teamExt.view?                  ← T2 view (one)
  └─ t3teamExt.attachments?           ← context attachments (existing)
```
...
- When `t3teamExt.displayText` is set: render it as the conversation body instead of
...
- When `t3teamExt.mdx === true` or any T1 widget is detected: route to
  `t3team-MdxChatRenderer.tsx` (new, additive) using `safe-mdx` with the resolved
  per-surface component map.
- When `t3teamExt.attachments` is set: render attachment cards from that structured payload.
  Context-bound sends and kickoff messages must store cards here; text syntax is only for the
  provider-facing prompt.
- When `t3teamExt.view` is set: render the T2 view in addition to (or instead of) the
...
- T2 view props are snapshot at write time into the `t3teamExt.view.props` payload —
...
- **AG-UI** is a runtime channel for cross-framework agent↔frontend interop. `t3team`
  runs agents in-process via the Codex Session Runtime and already has the equivalent
  bidirectional channel via `T3TeamToolBroker` + `t3teamExt`. AG-UI becomes
  interesting only if `t3team` exposes its agents to external frontends, at which
  point the same T1/T2/T3 payloads carry over its events.
- **A2UI** describes UI as declarative data. T2 views via `t3teamExt.view` already
...
## `t3team.widget.show`: the unified agent-facing widget tool (shipped slice)

This is the "agent-emits-widgets-inline" unification the References section flags as
missing. `t3team.widget.show` is the **single** broker tool an agent calls to put a widget
...
`POST /api/t3team/widget/tool-call`, which dispatches through the `T3TeamToolBroker`
...
(`apps/server/src/t3team-widgetCapabilityPolicy.ts`). Only html/svg is live today; mdx/tsx
...
- **New files (all additive):**
  - `apps/web/src/t3team/t3team-MdxChatRenderer.tsx`
  - `apps/web/src/t3team/t3team-inlineWidgetRegistry.ts`
  - `apps/web/src/t3team/widgets/t3team-*.tsx` (one per inline widget)
...
- **Modified upstream files (T1, Phases A–B):**
  - `MessagesTimeline.tsx` — already in the whitelist; the existing reason
    ...
    extends with: "Dispatch to t3team MDX renderer when `t3teamExt.mdx === true`."
...
- **Modified upstream files (T2b composer, Phase D):**
  - `packages/contracts/src/settings.ts` — already in the whitelist for
    t3team-prefixed client settings; this addition (`composerModelSelection:
...
2. Add `t3team-MdxChatRenderer.tsx` using `safe-mdx` with whitelist enforcement and
   fallback-on-failure semantics.
3. Add three T1 widgets: `<Callout>`, `<Badge>`, `<Sparkline>`.
4. Wire dispatch in `MessagesTimeline.tsx` behind `t3teamExt.mdx === true`.
...
3. ~~**MDX vs MDX-lite.**~~ **Resolved.** Two-shape model:
   ...
   - **T2 runtime invocation stays structured.** The agent does not free-type full
     MDX into a message at runtime — it emits a `t3teamExt.view` tool call with a
...
- [Epic 16 — Action Recipes](./16-action-recipes.md) — workflows, tool broker,
  message envelope, `t3teamExt` seam.
