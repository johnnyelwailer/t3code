...
2. **Project policy** — allow or deny providers for one `t3team` project.
...
into `.t3team/context/...`, a provider running in the same project workspace may be able to
...
```text
Project MOBILE has synced Jira context under .t3team/context/jira/...
...
- **No synced context** — start the provider in a clean workspace view that excludes
  `.t3team/context/<blocked-connector>`.
