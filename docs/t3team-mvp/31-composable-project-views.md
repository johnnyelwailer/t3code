...
3. **Shell owns data.** Blocks use shell-managed queryables, cache freshness, and tool
   broker permissions.
4. **Views are source artifacts.** Current examples use `.t3team/miniapps/` for
   project-local views. Under [Epic 36](./36-workspace-packs-and-distributions.md) this is
   transitional; generated/synced state should move to host app-data, with repo files only
   for explicitly project-owned source.
...
```text
/t3team/projects/:projectId?projectView=backlog
/t3team/projects/:projectId/views/:viewId
```
...
Backlog and My Work become the proof that this model is real. They should be expressible as
...
- Discover `.t3team/miniapps/*/miniapp.json` with `project.navView` placement.
...
- A sample `.t3team/miniapps/triage-dashboard` appears in project nav after enablement.
...
- Implement a `.t3team/miniapps/my-work` equivalent using public blocks.
...
- Implement `.t3team/miniapps/backlog` equivalent using public blocks.
...
- Generated view uses only `@t3team/blocks`.
...
3. Use only public `@t3team/blocks` and `@t3team/sdk` imports.
...
- Explicitly project-owned view source follows the project-local storage decision in
  [Epic 36](./36-workspace-packs-and-distributions.md); pack-installed views stay in host
  app-data.
