...
The MVP should be additive in the monorepo. Use `t3team` to mark additive packages and
...
```text
apps/web/src/t3team
packages/t3team-context
packages/t3team-recipes
packages/t3team-integrations-core
packages/t3team-integrations-atlassian
packages/t3team-artifacts
packages/t3team-skill-packs
packages/t3team-t3-adapter
```
...
pack-hosting logic should still follow the additive rule and prefer `t3team-` names where
new packages are needed.

The existing `apps/web`, `apps/server`, and core packages should remain the upstream
engine. `t3team` can duplicate UI patterns, but should avoid scattering
`t3team` behavior through existing files.

## T3 Adapter Boundary

`packages/t3team-t3-adapter` is the only package allowed to depend on unstable T3
...
- map T3 project/thread state into `t3team` state
- normalize current T3 assumptions such as `workspaceRoot`

Rule:

```text
apps/web/src/t3team -> packages/t3team-t3-adapter -> existing T3 internals
```

No other `t3team` package should deep import existing T3 internals.

## Project Context Package

`packages/t3team-context` defines the shared model.
...
`packages/t3team-integrations-core` defines service-agnostic interfaces.
...
`t3team` should create a local workspace automatically when the project does
not start from a user-selected folder.

Default layout:

```text
~/Library/Application Support/T3 Code/t3team/projects/<project-id>/
  project.json
  recipes/
  sources/
  plans/
...
T3 over a `t3team`-specific patch.
...
`t3team` extends `t3` without forking it. The enforcement lives in the additive guard
(`.t3team-additive-guard.json`, runner `t3team-additive-guard.mjs`); the canonical
allowlist and reason log is [`docs/t3team-additive-whitelist.md`](../t3team-additive-whitelist.md).
...
> all the real logic in `t3team-`-prefixed files. Do not grow the allowlist if a smaller
> seam exists.**
...
- **Optional field** — a single optional field on an upstream type, ideally generically
  shaped or namespaced under a `t3teamExt?` key.
- **Insertion component** — a `<T3TeamSomething />` element rendered at one fixed point.
- **Marker key** — a reserved key inside an existing extension dictionary (e.g. a
  client-settings record).

Anything richer than these belongs in a t3team-prefixed file that the seam invokes.

### Proven examples on the allowlist

- `apps/web/src/components/ChatView.tsx` — `composerContextAttachmentSlot?: ReactNode`
  prop plus a single `onSend` read. All chip behaviour lives in `t3team-`-prefixed files.
...
- `apps/web/src/components/settings/SettingsPanels.tsx` — `<T3TeamProjectSetupSetting />`
  insertion component.
- `packages/contracts/src/settings.ts` — `t3teamStoredProjectsJson` /
  `t3teamStoredSidebarPinsJson` reserved keys.

All of these are tiny: optional, generically-shaped, and auto-merge-safe. The complete
behaviour lives in `t3team-`-prefixed code that imports the seam.

### Applying the rule to new work

When designing an extension, ask in this order:

1. Can it live entirely in a `t3team-`-prefixed file inside a whitelisted package
   (`packages/project-recipes/**`, `packages/project-context/**`,
   `packages/integrations-*/**`, `packages/t3team-skill-packs/**`,
   `apps/web/src/routes/t3team.tsx`)? **Prefer this. No allowlist growth.**
...
4. Is the seam generically shaped (plausibly upstreamable) rather than t3team-specific?
   Generic shapes age better.
5. Each new `allowedModifiedFiles` entry needs a one-line reason in
   [`docs/t3team-additive-whitelist.md`](../t3team-additive-whitelist.md).

The 200 LOC per-file ceiling forces splitting by concern. Plan for many small focused
`t3team-`-prefixed files rather than one large file per feature.
...
export type Message = {
  ...
  t3teamExt?: T3TeamMessageExt; // the entire seam
};

// packages/contracts/src/t3team-message-ext.ts  (new, prefix-compliant — no allowlist entry needed)
...
provider adapters, the view-in-message renderer — lives in `t3team-`-prefixed files in
