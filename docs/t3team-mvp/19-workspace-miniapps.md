...
are the four shared primitives all `t3team` automation is built on; this epic owns Views.
...
extensions. It may be git backed like any project workspace. When `t3team` creates a
...
"sidebar" in t3team's vocabulary) is a **composition of topic-grouped sections**, not a
...
`TicketKickoffPanel` (covered by `t3team-createProjectBootstrap.test.ts`).
...
Storybook is set up for `t3team` sidecar and recipe presentation. Build and review section
chrome in isolation before full dashboard wiring.

- Config: `apps/web/src/t3team/storybook/t3team-storybook-main.ts`
- Dev: `pnpm storybook` from repo root
- Static build: `pnpm storybook:build`
- Stories colocated: `apps/web/src/t3team/**/*.stories.tsx`
- Sidecar fixtures: `apps/web/src/t3team/t3team-sidecarStoryFixtures.ts`
...
```ts
// @t3team/sdk
import { defineSidecarSection } from "@t3team/sdk";
...
client-settings key
(`t3teamStoredSidecarCompositionJson`; `packages/contracts/src/settings.ts` is already
...
| **Hide item**          | Universal | per-user | Item never shows for this user. Persisted in `t3teamStoredSidecarCompositionJson` under `itemHides`. Per-project hide remains **deferred**.             |
...
| Action                | Source    | Scope    | Notes                                                                                                                                       |
| --------------------- | --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hide section**      | Universal | per-user | Persisted in `t3teamStoredSidecarCompositionJson`.                                                                                          |
  ...
  itemActions: (item) => [
    {
      ...
      icon: "external-link",
      run: { kind: "tool", toolName: "t3team.github.open_pull_request", input: { id: item.id } },
    },
    {
      id: "mark-reviewed",
      label: "Mark as reviewed",
      icon: "check",
      run: { kind: "tool", toolName: "t3team.github.mark_reviewed", input: { id: item.id } },
    },
  ],
  sectionActions: () => [
    {
      ...
      label: "Refresh now",
      icon: "refresh-cw",
      run: { kind: "tool", toolName: "t3team.github.refresh_activity_context" },
    },
  ],
});
```
...
- **Developer-mode actions** (Open source, Duplicate as new recipe, View dependencies).
  Deferred. Likely arrives as a `t3team.developerMode` setting that toggles a separate
...
The `@t3team/sdk` exposes a small set of **`define*` helpers**, one per
...
add a qualifier from the t3team surface vocabulary (`sidecar` / `workitem` / `dashboard` /
...
real surfaces in the t3team UI — bounded growth, no string-literal landmines.
...
- load imports only from `@t3team/sdk` and approved runtime shims
...
3. Agent writes `.t3team/miniapps/project-health/miniapp.json`.
4. Agent writes `App.tsx` using `@t3team/sdk`.
