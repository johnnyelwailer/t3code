...
A **distribution** is an installer/build/channel that ships core `t3team` plus zero or
more bundled packs.
...
- explicit project manifests such as `t3team.project.json`
...
```text
<app-data>/t3team/packs/
  distribution/
  global/
  remote/<endpoint-id>/
  cache/

<app-data>/t3team/users/<user-id>/packs/
  personal/
```
...
```text
<app-data>/t3team/projects/<project-id>/
  context/
  local-source/
  runs/
  pack-bindings.json
...
<project-workspace>/
  t3team.project.json   # optional explicit repo-owned binding/manifest
```

Current specs and code still mention `.t3team/...` for project-local recipes, miniapps,
...
or synced state, with a visible repo file only when the project intentionally owns t3team
...
- **Project storage posture:** `.t3team` is transitional. Prefer host app-data keyed by
  project id for generated/synced state and optional explicit repo manifests for
  project-owned configuration.
- **Repo binding posture:** if a repo needs to own t3team configuration, use a visible
  `t3team.project.json` file. Do not introduce a new hidden project directory as the target
  storage model.
