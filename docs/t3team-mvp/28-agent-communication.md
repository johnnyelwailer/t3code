...
`renderAgentsMd` in `apps/server/src/t3team-projectSetupContent.ts`, kept fresh by the
...
The current renderer (`t3team-projectSetupContent.ts`) already has good bones — a
...
- [Epic 27: Scheduled Workflows](./27-scheduled-workflows.md) — routines, offered as "run
  this every…".
- `apps/server/src/t3team-projectSetupContent.ts` — `renderAgentsMd`, the single source
