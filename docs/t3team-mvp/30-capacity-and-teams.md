...
server `t3team-tempo.ts`, 2026-06-11)
Owner: PJ
...
3. Agent tool `t3team.project.open_dashboard_mode` gains the `capacity` target.

## 3. Capacity resolution pipeline

One server-side function (extending `t3team-tempo.ts` into `t3team-capacity.ts`):
...
## 4. Data model (backend SQLite, migration `t3team-035_CapacityTeams`)

```sql
t3team_teams              (id, name, project_id NULL,        -- NULL = global team
                           is_default INTEGER, created_at, updated_at)
t3team_team_members       (team_id, account_id, display_name, role NULL, sort_order)
t3team_capacity_off_days  (id, account_id, date_from, date_to, label,
                           seconds_per_day NULL,             -- NULL = full day off
                           created_by, created_at)
t3team_capacity_settings  (scope,                            -- 'global' | account_id
...
`/api/t3team/capacity/*` route family (same pattern as the tempo routes):
...
`/capacity/resolve` wraps the shipped `loadT3TeamTempoCapacity` as stage 1 and applies
overlays; `/capacity/board` joins it with the backlog cache (sprint windows + per-
assignee load sums come from `t3team_atlassian_backlog_issues`) so the web client gets
...
- A setup card explains the three source tiers and lets the user pick one:
  1. **Connect Tempo (OAuth)** — standard flow per 29 §10.2: redirect URI
     `${origin}/oauth/callback` (same convention as `t3team-useAtlassianOAuth`),
     token exchange server-side, refresh handled like the Atlassian OAuth store.
  2. **Tempo API token** — paste field posting to `/api/t3team/tempo/token`
     (shipped).
...
```
T3TEAM_TEMPO_CLIENT_ID=...        # OAuth app client id
T3TEAM_TEMPO_CLIENT_SECRET=...    # server-side only, never shipped to the client
T3TEAM_TEMPO_API_TOKEN=...        # alternative: static token, skips OAuth entirely
```

Exchanged/refreshed Tempo OAuth tokens persist next to the Atlassian auths
(`secrets/t3team-tempo-auth.bin`), superseding the plain token file when present.
...
```text
# read (safe)
t3team.capacity.read_board            # current grid as structured data
t3team.capacity.read_person           # one person's breakdown incl. off-days
t3team.capacity.list_teams

# view-state (safe)
t3team.capacity.open_view             # open capacity mode, optional team/sprint focus
t3team.capacity.filter_by_team        # also usable from backlog (sets team filter)
t3team.capacity.focus_person
...
t3team.capacity.set_off_days          # {accountId, from, to, label, halfDay?}
t3team.capacity.remove_off_days
t3team.capacity.set_overhead          # {accountId | "global", percent}
t3team.capacity.create_team
t3team.capacity.set_team_members
t3team.capacity.set_default_team
```
