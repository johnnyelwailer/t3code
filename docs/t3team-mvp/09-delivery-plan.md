...
- skeleton `apps/web/src/t3team`
- mock integration provider
- one managed workspace from mock project

Validation:

- `t3team` creates a project without selecting a directory
- `t3team` can start a T3 thread through adapter
...
- Keep all T3 coupling inside `packages/t3team-t3-adapter`.
- Treat `workspaceRoot` as internal in `t3team` UI.
