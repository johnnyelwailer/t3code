# T3Team Recipes/Skills/Tools Planning
...
2. **Integration Platform** (in place, Atlassian proof connector first)
   ...
   - Atlassian routes in server (`t3team-atlassian-routes.ts`)
...
2. **Create `packages/t3team-skill-tools`**
...
4. **Create tool handler registry in server**
   - `packages/server/src/t3team-tool-handlers/`
...
1. Create `packages/t3team-skill-tools` with minimal tool definitions
