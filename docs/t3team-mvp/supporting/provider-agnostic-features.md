...
are connectors in the pack-driven t3team specs, even where older code fields still use
`provider`.
...
**File**: [apps/server/src/t3team-artifact-store.ts](apps/server/src/t3team-artifact-store.ts) (if it exists) or similar
...
1. Create `packages/t3team-skill-tools/src/tools/integration.resources.list.ts`
