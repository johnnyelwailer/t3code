...
```tsx
// apps/web/src/t3team/t3team-CreateProjectDialog.tsx line 101
...
```ts
// apps/web/src/t3team/hooks/t3team-useCreateProject.ts line 74
...
```ts
// apps/web/src/t3team/backend/t3team-t3Backend.ts line 92
const atlassian = {
  async connectBasic(input: AtlassianBasicConnectInput) {
    const response = await postJson(
      httpBaseUrl,
      "/api/t3team/atlassian/connect/basic", // ← WHAT URL?
      {
        auth: {
          kind: "basic",
...
```ts
// apps/server/src/t3team-atlassian-routes.ts line 32
export const t3teamAtlassianConnectBasicRouteLayer = HttpRouter.add(
  "POST",
  "/api/t3team/atlassian/connect/basic", // ← MATCHES THE URL
  Effect.gen(function* () {
...
```ts
// apps/web/src/t3team/hooks/t3team-useCreateProject.ts line 127
...
```ts
// apps/web/src/t3team/backend/t3team-t3Backend.ts line 130
async listProjects(account: IntegrationAccountRef): Promise<ReadonlyArray<ExternalProject>> {
  const response = await postJson(
    httpBaseUrl,
    "/api/t3team/atlassian/projects",  // ← ANOTHER HTTP CALL
...
```ts
// apps/server/src/t3team-atlassian-routes.ts line 109
...
```ts
// apps/web/src/t3team/hooks/t3team-useCreateProject.ts line 155
...
```ts
// apps/web/src/t3team/backend/t3team-t3Backend.ts line 165
async createProject(input: CreateProjectInput): Promise<ProjectShellProject> {
  const response = await postJson(
    httpBaseUrl,
    "/api/t3team/projects/create",  // ← YET ANOTHER HTTP CALL
...
POST /api/t3team/projects/create
{
...
- `POST /api/t3team/atlassian/connect/basic`
- `POST /api/t3team/atlassian/projects`
- `POST /api/t3team/atlassian/resources`
- etc.
...
2. Tool definitions in `packages/t3team-skill-tools`
