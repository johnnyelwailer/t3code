...
`t3team.runtime.resource_pressure` (runtime flag `NEXI_FF_RESOURCE_PRESSURE`, default off; answers
`enabled: false` and samples nothing when off) returns the host's memory-pressure level (`ok` /
`warn` / `critical`), machine memory, RSS split into the app itself (server, window), what it
spawned (agent CLIs, terminals, their children) and the rest of the machine, worktree count, the
top memory-consuming T3 processes, recent persisted pressure transitions and a `stopSpawning` flag
(true at `critical`). While `stopSpawning` is true the host refuses `t3team.thread.start_child` and
`t3team.orchestration.run` with an explanatory error (dispatch backoff). It reads the server's latest bounded
