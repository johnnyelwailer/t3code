...
`t3team` should reuse this model. External resources should use `@`, not introduce a
new primary symbol.
...
`t3team` should extend this instead of creating a separate resource picker model.
...
Auto-detected references must always resolve inside the active `t3team` project scope.
...
`t3team` should mirror that mental model, but with stricter project scoping:
...
Use the shared `t3team` resource model.
...
1. Open a Jira-backed `t3team` project.
2. Focus composer.
3. Type `@`.
