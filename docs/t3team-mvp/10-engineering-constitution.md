...
`t3team` should reuse T3 Code's existing engineering conventions wherever they
exist. This constitution is an overlay for the new `t3team` packages and app, not a
...
`t3team` must start from the existing T3 Code shell and UI as its baseline.
...
then adapt that baseline for `t3team` workflows.
...
3. Copy existing T3 UI into `apps/web/src/t3team` when extraction would slow the MVP.
4. Adapt the copied/imported baseline for `t3team` needs.
...
Any new `t3team` UI surface should be able to answer:

- Which existing T3 shell surface did this start from?
- What did we keep unchanged?
- What did we adapt for the `t3team` use case?
- Why was new UI necessary, if any?

## Package Boundaries

`t3team` code lives in additive packages. New files must either use the `t3team-` /
`t3team.` prefix or sit in a whitelisted package path (see
[`docs/t3team-additive-whitelist.md`](../t3team-additive-whitelist.md) and the additive
extension pattern in [Epic 02](./02-additive-architecture.md#additive-extension-pattern)).

Current `t3team` additive packages on disk:

- `apps/web/src/t3team`
- `packages/project-context` (formerly proposed as `t3team-context`)
- `packages/project-recipes` (formerly proposed as `t3team-recipes`)
- `packages/integrations-core` (formerly proposed as `t3team-integrations-core`)
- `packages/integrations-atlassian` (formerly proposed as `t3team-integrations-atlassian`)
- `packages/t3team-skill-packs`
- `@t3team/sdk` — the additive workflow/tool authoring SDK (Epic 25: `defineWorkflow`,
...
`t3team-` prefix unless extending one of these existing packages.
...
- New cross-cutting concerns follow the additive extension pattern: minimal optional seam
  upstream + all logic in `t3team-`-prefixed files. Do not grow the modified-files
...
Target coverage for `t3team` packages: 90-100%.
...
Storybook is set up for `t3team` UI. Run from repo root:

- Dev: `pnpm storybook`
- Static build: `pnpm storybook:build`

Config: `apps/web/src/t3team/storybook/t3team-storybook-main.ts`. Stories live at
`apps/web/src/t3team/**/*.stories.tsx`. Prefer Storybook-first iteration for sidecar
...
Atlassian-specific code belongs in `packages/t3team-integrations-atlassian`. The rest of the
...
code. UI components and hooks may call t3team backend methods, but must not
...
`t3team` is an app for repeated work, not a marketing page.
...
For `t3team` work, browser validation should cover the affected path, for example:
