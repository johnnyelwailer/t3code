...
**live on every poll** through a second, parallel `listResources` path that _bypasses the
existing SQLite cache entirely_, then mirrors the whole `ResourcePage` into `localStorage`.
...
**Generalize this shape.** Every data cache becomes: _raw table(s)_ + _view/projection
row(s) keyed by scope_ + _fingerprint_ + _(optional) background sync_. The client hook
...
> Note: the _preferences_ for the My Work / Backlog dashboards stay local (they are UI
> state). Only the _data_ those views render moves to the server.
...
2. **Whole-project raw mirror (decided).** Instead of per-view syncs, mirror the **entire
   ...
   (below) keeps fresh cheaply. Today's backlog cursor-walk becomes the _initial backfill_
...
5. **Fingerprint stays the contract.** Keep `toT3TeamPollResult` / `createT3TeamPollFingerprint`.
   The change is _where the payload lives when `unchanged`_ — server SQLite, not the browser.
...
8. **Freshness = incremental watermark poll (decided).** Realizes doc 18's deferred Phase 3
   ...
   `issue_id`. **Deletions/moves** (issues leaving the project) are _not_ caught by
...
9. **Rate-limit discipline (Atlassian/GitHub throttle quickly).** Hard constraint, not a
   ...
   - **No fixed silent caps.** The current `limit ?? 50` _is_ the reported bug (§1) — paging
...
Out of scope for this work, but worth designing _toward_. In an enterprise rollout the
...
the contract clean — _the server owns SQLite; clients only talk to the server's projection
endpoints; no client ever touches raw provider data or its own data cache_ — then extracting
