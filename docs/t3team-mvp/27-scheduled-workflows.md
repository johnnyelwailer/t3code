...
```ts
// .t3team/routines/weekly-triage.workflow.ts
import { Schema } from "effect";
import { jiraRead, jiraWrite } from "@t3team/sdk/groups";
import { nextWeekday } from "@t3team/sdk/time";
...
- **Imported — allowlisted pure modules only:** `Schema` from `effect`, capability groups
  from `@t3team/sdk/groups`, and the time helpers from `@t3team/sdk/time`.
...
The SDK wraps the common cadences in an allowlisted pure module, `@t3team/sdk/time`, so
authors don't hand-roll them:

```ts
import { nextWeekday, nextCron } from "@t3team/sdk/time";
...
bypasses the journal and diverges on replay. So `@t3team/sdk/time` helpers are pure
...
- **Interactive-thread model** — the thread-as-canvas, composer-as-universal-input,
  ...
  pass + the project memory (`project_t3team-recipes-architecture`), **not yet a standalone
