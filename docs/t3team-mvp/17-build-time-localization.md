...
Add a pack-aware localization path for `t3team` without hand-editing existing T3 Code UI
source files.
...
`t3team` surface while preserving the upstream source tree. Under the pack-driven vision,
...
- heavier runtime model than a narrow `t3team` experiment likely needs
...
The spike should only target `apps/web/src/t3team/**` first. Do not translate the whole
T3 Code shell in the first pass.

## Proposed Pipeline

1. Extract
   - Scan `apps/web/src/t3team/**/*.{ts,tsx}`.
...
2. Catalog
   - Generate `apps/web/locales/t3team/en.po` or `en.json`.
...
4. Compile
   - Compile catalogs into generated modules under
     `apps/web/src/t3team/localization/generated/`.
...
5. Transform
   ...
   - Example:
     ...
     ```tsx
     <Button>{t3teamMessage("hash", "Create project")}</Button>
     ```
...
6. Select Locale
   - Start with an environment variable:
     ...
     ```bash
     T3TEAM_LOCALE=de bun --filter @t3tools/web build
     ```

   - Later, add a client setting only after the build-time path is proven.

7. Verify
   - Run browser checks on the `t3team` entry surfaces.
...
- `node t3team-additive-guard.mjs` passes
- English build still renders unchanged
- pilot locale build renders translated `t3team` chrome
...
1. Keep zero-source build-time localization for `t3team` only.
2. Switch to explicit source-level i18n for new `t3team` components.
...
new `t3team` UI if localization becomes a real product requirement.
