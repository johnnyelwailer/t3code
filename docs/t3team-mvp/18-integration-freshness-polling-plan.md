...
`t3team` needs fresher connector data (GitHub and Jira in the current proof slice) without
violating the additive constraints in the constitution.

The first rollout should avoid non-`t3team` files and avoid introducing new core server
or websocket seams. That means polling first, through existing `t3team` backend routes,
...
- allowed: `docs/t3team-mvp/**`
- allowed: `apps/web/src/t3team/**`
- allowed when needed later: `apps/server/src/t3team-*.ts`
- allowed when needed later: `packages/integrations-atlassian/src/provider.ts`
- not in phase 1 without explicit follow-up approval: core files such as `apps/server/src/server.ts`, `apps/server/src/ws.ts`, or other non-`t3team` runtime seams
...
- browser UI already calls `t3team` backend routes
- backend routes already own third-party access
- GitHub and Jira read surfaces already exist in `t3team` hooks; long term these are
...
- shared `t3team` polling/freshness helper in `apps/web/src/t3team/hooks`
- cache max-age support in the `t3team` integration cache
...
- use existing `t3team` HTTP endpoints only
- do not call third-party APIs from browser code
- do not add new non-`t3team` subscriptions or runtime layers
...
## Phase 2: Route Efficiency Inside Existing `t3team` Files

Deliverables:

- GitHub route-level conditional fetch improvements inside `apps/server/src/t3team-github-*.ts`
- Jira delta-oriented list helper inside `packages/integrations-atlassian/src/provider.ts`
- optional response metadata for next recommended poll cadence if the `t3team` routes need it

Goals:

- reduce repeated third-party reads while keeping the same browser contract
- keep route and provider changes limited to `t3team`-owned integration files
...
- a dedicated `t3team` sync service on the server
...
not currently isolated to `t3team` files.
...
- we approve a narrow additive server seam for `t3team` integration events

## Test Strategy

Coverage target for this work remains 90-100% on the new `t3team` helper modules.
...
- open a `t3team` project with GitHub activity and confirm it refreshes while visible
