...
Current `t3team` GitHub integration is context-first, not workspace-first.

That is enough for linking GitHub activity to Jira work items and for attaching pull
request bundles to chat, but it is not enough for doing review work inside `t3team`.
...
- stay additive to the current `t3team` boundaries and provider contracts
...
checkpoint/thread diff viewer, not a t3team pull-request viewer. We should reuse the
underlying rendering stack and shell patterns, not import the whole component unchanged.

### Reuse Now

Existing `t3team` GitHub read pipeline already covers much of the raw data work:

- `apps/server/src/t3team-github-routes-pr-context.ts`
  - loads PR metadata, files, reviews, issue comments, review comments, commits, whole
    diff text, and file snapshots
- `apps/server/src/t3team-github-routes-pr-files.ts`
  - already fetches base/head file snapshots for changed files
- `apps/server/src/t3team-github-routes.ts`
  - already exposes additive HTTP route seams under `/api/t3team/github/*`
- `apps/server/src/t3team-github-inbox-routes.ts`
  - already provides a polling-oriented GitHub activity surface

Existing `t3team` artifact and summary generation is also reusable:

- `apps/web/src/t3team/t3team-githubPullRequestContextBundle.ts`
- `apps/web/src/t3team/t3team-githubPullRequestContextRender.ts`
- `apps/web/src/t3team/t3team-githubPullRequestContextBundleRender.ts`
- `apps/web/src/t3team/t3team-githubPullRequestContextAssets.ts`

These files already prove that `t3team` can normalize PR context into durable artifacts,
summary markdown, asset downloads, and add-to-chat payloads.

Existing `t3team` UI shell/state patterns are also directly relevant:

- `apps/web/src/t3team/t3team-AppMainContent.tsx`
  - current t3team route/view resolution surface
- `apps/web/src/t3team/hooks/t3team-usePersistedRouteState.ts`
  - persisted route/search state pattern for t3team-specific views
- `apps/web/src/t3team/t3team-ResizableRightSidebarLayout.tsx`
  - ready-made dense split-view layout for detail + right-side sidecar shells
- `apps/web/src/t3team/components/ui/t3team-*`
  - additive t3team UI primitives already aligned with the current shell
...
`apps/web/src/components/DiffPanel.tsx` is not a direct `t3team` PR viewer.
...
- reuse the underlying `@pierre/diffs` stack, worker pool, shell styling, and toggle
  patterns
- avoid importing the full `DiffPanel` into `t3team`
- either extract one small generic patch-render primitive from it, or build a
  `t3team`-prefixed PR diff viewer that copies only the generic rendering slice

Likewise, the base source-control UI should not be pulled wholesale into `t3team` PR
...
- first-class PR detail route and view type in `t3team`
...
- `@tanstack/react-pacer` for debounced file filtering, search, and expensive derived-state
  recalculation
- existing `t3team` UI primitives and layout components for detail/side-panel shells
...
- keep new GitHub endpoints in `apps/server/src/t3team-github-*.ts`
...
- reuse `GitWorkflowService` and `GitManager` for repository-aware PR authoring/worktree
  actions instead of rebuilding branch/PR orchestration inside `t3team`
...
- new runtime code should live in `apps/web/src/t3team/**`
- add a new `t3team` PR route/view type inside the existing `t3team` view-resolution flow,
  not inside the base chat diff route
- use `t3team-usePersistedRouteState` and existing split-view layout patterns for PR-level
  navigation/search/filter persistence
- implement the default PR workspace as a composition of reusable `t3team` blocks first,
...
- new GitHub HTTP routes should live in `apps/server/src/t3team-github-*.ts`
- extend the existing `t3team-github-routes.ts` family instead of scattering GitHub logic
  across generic server files
- keep third-party reads behind those backend routes; browser code should continue to call
  `t3team` backend methods only
...
- do not edit `apps/web/src/components/DiffPanel.tsx` unless a tiny reusable extraction is
  clearly lower risk than copying the generic render slice into a `t3team` file
- do not edit generic Git/source-control UI for `t3team` presentation concerns
- reuse existing PR lookup/creation/worktree services through thin wrappers or adapters
  instead of duplicating them in `t3team`
- if one upstream seam is unavoidable, it should be small and plausibly upstreamable, such
  as a generic patch-render primitive or a single merged `t3team` GitHub route layer seam

### Practical Implementation Order

1. Stay inside `docs/t3team-mvp/**`, `apps/web/src/t3team/**`, and
   `apps/server/src/t3team-github-*.ts` for the first implementation slice.
2. Land the PR detail shell first by reusing the current pull-request-context route and
   current artifact vocabulary.
3. Add diff-manifest and diff-chunk routes next, then build the `t3team` PR diff viewer on
...
- users can stay in `t3team` for normal review work without opening GitHub
...
- PR title, body, branch name, commit messages, and repository/project configured key
  patterns
- current `t3team` project backlog cache and rich work-item context bundle
...
Mutation policy stays consistent with the broader `t3team` model:
...
Users should be able to ask `t3team` to help create a PR from the current branch,
...
- open a large PR without leaving `t3team`
