# t3team Vision — A Pack-Driven Work Platform

## In One Paragraph

`t3team` is a team-based, project-aware agentic shell layered on top of T3 Code. It brings
...
- Core `t3team` is **building blocks**, not an opinionated work app.
- `t3team` is a **shared team shell**, not just a better one-person chat UI.
...
- A company can make `t3team` feel like its own internal work app without forking the
  shell.
...
type WorkspacePackManifest = {
  ...
  publisher?: string;
  compatibility: {
    t3teamCore: string; // semver range
...
- Do not make core `t3team` a Jira, Confluence, QA, or support product.
...
Any company, team, or individual can make `t3team` their workbench by composing packs:
